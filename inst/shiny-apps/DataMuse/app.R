    ...
    shiny::fluidRow(
      shiny::column(
        ...
        shiny::fluidRow(
          shiny::column(
            width = 12L,
            DataMuse::SideBarUI(id = "SideBarUIID", AppWidth = 12L)
          ) # column
        ) # fluid Row
      ),# end column width =  2L
                ...
                shinyjqui::sortableTabsetPanel(
                  ...
                  shiny::tabPanel(
                    ...
                    shiny::fluidRow(
                      ...
                      DataMuse::HomePage(id = 'HomePageID'))),
...
server <- function(input, output, session) {
  ...
  StockSymbolsData <- NULL  #DataMuse:::LoadCSV(Infile = system.file(package = "DataMuse", "shiny-apps", "DataMuse", "ticker_data.csv"))
  ...
  shiny::observeEvent(input$PostGRE_Modal, {
    ...
    if(length(PostGRE_Host) > 0L && length(PostGRE_Port) > 0L && length(PostGRE_User) > 0L && length(PostGRE_Password) > 0L) {
      PostGRE_DBNames <- DataMuse:::DM.pgListDatabases(Host = PostGRE_Host, Port = PostGRE_Port, User = PostGRE_User, Password = PostGRE_Password)
    } else {
      PostGRE_DBNames <- NULL
    }

    # Dispatch modal
    DataMuse:::PostGRE_Modal_Fun(
      id = 'PostGREDMID',
    ...
    if(length(PostGRE_DBNames) > 0L) {
      ...
      shiny::observeEvent(DBName(), {
        x <- tryCatch({DataMuse::DM.pgListTables(DataBase = shiny::req(DBName()), Host = PostGRE_Host, Port = PostGRE_Port, User = PostGRE_User, Password = PostGRE_Password)}, error = function(x) NULL)
        if(length(x) == 0) x <- NULL
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'PostGRE_PullTable_Table', Label = NULL, Choices = x, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1L, CloseAfterSelect = FALSE, Debug = Debug)
        ...
        ColNames <- shiny::eventReactive(eventExpr = shiny::req(TableName()), {
          tryCatch({DataMuse::DM.pgTableColnames(Host = PostGRE_Host, DataBase = DBName(), Table = TableName(), User = PostGRE_User, Port = PostGRE_Port, Password = PostGRE_Password)}, error = function(x) NULL)
        })
        ...
        shiny::observeEvent(shiny::req(ColNames()), {
          ...
          DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'PostGRE_PullTable_Columns', Label = NULL, Choices = x, SelectedDefault = NULL, Debug = Debug)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'PostGRE_PullTable_GroupByColumns', Label = NULL, Choices = x, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 100L, CloseAfterSelect = FALSE, Debug = Debug)
        }, ignoreInit = TRUE)
      ...
      shiny::observeEvent(shiny::req(DropDBName()), {
        ...
        x <- DataMuse::DM.pgListTables(DataBase = DBNameSelected, Host = PostGRE_Host, Port = PostGRE_Port, User = PostGRE_User, Password = PostGRE_Password)
        if(length(x) == 0L) x <- NULL
        DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'PostGRE_DropTable_Table', Label = NULL, Choices = sort(x), SelectedDefault = NULL, Multiple = TRUE, Debug = Debug)
      }, ignoreInit = TRUE)
    }
  })
  ...
  shiny::observeEvent(input$Shrink_Modal, {
    ...
    shiny::observeEvent(AggregateData_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AggregateData_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AggregateData_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Aggregate_Columns', Label='Aggregate Columns', Choices = ChoiceList, SelectedDefault = if(length(input$Aggregate_Columns) > 0L) input$Aggregate_Columns else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Aggregate_ByVariables', Label='Aggregate By-Columns', Choices = ChoiceList, SelectedDefault = if(length(input$Aggregate_ByVariables) > 0L) input$Aggregate_ByVariables else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Aggregate_DateVariable', Label='Aggregate Date Column', Choices = ChoiceList, SelectedDefault = if(length(input$Aggregate_DateVariable) > 0L) input$Aggregate_DateVariable else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(SubData(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(SubData()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(SubData(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterVariable1', Label = 'Filter Variable 1', Choices = ChoiceList, SelectedDefault = if(length(input$SubsetData_FilterVariable1) > 0L) input$SubsetData_FilterVariable1 else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterVariable2', Label = 'Filter Variable 2', Choices = ChoiceList, SelectedDefault = if(length(input$SubsetData_FilterVariable2) > 0L) input$SubsetData_FilterVariable2 else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterVariable3', Label = 'Filter Variable 3', Choices = ChoiceList, SelectedDefault = if(length(input$SubsetData_FilterVariable3) > 0L) input$SubsetData_FilterVariable3 else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterVariable4', Label = 'Filter Variable 4', Choices = ChoiceList, SelectedDefault = if(length(input$SubsetData_FilterVariable4) > 0L) input$SubsetData_FilterVariable4 else NULL, Multiple = TRUE, MaxVars = 1L)
      ...
      shiny::observeEvent(SubsetData_FilterVariable1(), {
        ...
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = SubsetData_FilterVariable1())$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_1_1', Label = "Value 1", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_1_1) > 0L) input$SubsetData_FilterValue_1_1 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = SubsetData_FilterVariable1())$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_1_2', Label = "Value 2", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_1_2) > 0L) input$SubsetData_FilterValue_1_2 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
      })
      ...
      shiny::observeEvent(SubsetData_FilterVariable2(), {
        ...
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = zzz)$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_2_1', Label = "Value 1", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_2_1) > 0L) input$SubsetData_FilterValue_2_1 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = zzz)$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_2_2', Label = "Value 2", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_2_2) > 0L) input$SubsetData_FilterValue_2_2 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
      })
      ...
      shiny::observeEvent(SubsetData_FilterVariable3(), {
        ...
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = zzz)$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_3_1', Label = "Value 1", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_3_1) > 0L) input$SubsetData_FilterValue_3_1 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = zzz)$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_3_2', Label = "Value 2", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_3_2) > 0L) input$SubsetData_FilterValue_3_2 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
      })
      ...
      shiny::observeEvent(SubsetData_FilterVariable4(), {
        ...
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = zzz)$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_4_1', Label = "Value 1", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_4_1) > 0L) input$SubsetData_FilterValue_4_1 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
        choices <- tryCatch({DataMuse:::KeyVarsInit(SubData(), VarName = zzz)$ChoiceInput}, error = function(x) NULL)
        DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID = 'SubsetData_FilterValue_4_2', Label = "Value 2", Choices = choices, SelectedDefault = if(length(input$SubsetData_FilterValue_4_2) > 0L) input$SubsetData_FilterValue_4_2 else NULL, Multiple = TRUE, MaxVars = 100L, Debug = Debug)
      })
    })
    ...
    shiny::observeEvent(DeleteVariables_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(DeleteVariables_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(DeleteVariables_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='DeleteVariables', Label='Delete Columns', Choices = ChoiceList, SelectedDefault = if(length(input$DeleteVariables) > 0L) input$DeleteVariables else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(SampleData_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(SampleData_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(SampleData_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='SampleData_StratifyColumns', Label='Stratify Variables', Choices = ChoiceList, SelectedDefault = if(length(input$SampleData_StratifyColumns) > 0L) input$SampleData_StratifyColumns else NULL, Multiple = TRUE)
    })
  })
  ...
  shiny::observeEvent(input$Grow_Modal, {
    ...
    shiny::observeEvent(JoinData_DataReactive1(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(JoinData_DataReactive1()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(JoinData_DataReactive1(), Types = ColTypes[i])
      DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID='JoinData_ByXVariables', Label='Left Table ByVars', Choices = ChoiceList, Selected = if(length(input$JoinData_ByXVariables) > 0L) input$JoinData_ByXVariables else NULL, Multiple = TRUE, MaxVars = 1000, Debug = Debug)
    })
    ...
    shiny::observeEvent(JoinData_DataReactive2(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(JoinData_DataReactive2()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(JoinData_DataReactive2(), Types = ColTypes[i])
      DataMuse:::SelectizeInput(session = session, Update = TRUE, InputID='JoinData_ByYVariables', Label='Right Table ByVars', Choices = ChoiceList, Selected = if(length(input$JoinData_ByYVariables) > 0L) input$JoinData_ByYVariables else NULL, Multiple = TRUE, MaxVars = 1000, Debug = Debug)
    })
  ...
  shiny::observeEvent(input$DataSets_Modal, {
    ...
    shiny::observeEvent(AutoDataPartition_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoDataPartition_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoDataPartition_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDataPartition_StratifyColumnNames', Label='Stratify Variable', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDataPartition_StratifyColumnNames) > 0L) input$AutoDataPartition_StratifyColumnNames else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDataPartition_TimeColumnName', Label='Date Variable', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDataPartition_TimeColumnName) > 0L) input$AutoDataPartition_TimeColumnName else NULL, Multiple = TRUE, Debug = Debug)
    })
    ...
    shiny::observeEvent(SortData_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(SortData_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(SortData_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='SortData_SortColumns', Label='Sort Columns', Choices = ChoiceList, SelectedDefault = if(length(input$SortData_SortColumns) > 0L) input$SortData_SortColumns else NULL, Multiple = TRUE, Debug = Debug)
    })
    ...
    shiny::observeEvent(ModelDataPrep_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(ModelDataPrep_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(ModelDataPrep_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='ModelDataPrep_IgnoreCols', Label='Skip Columns', Choices = ChoiceList, SelectedDefault = if(length(input$ModelDataPrep_IgnoreCols) > 0L) input$ModelDataPrep_IgnoreCols else NULL, Multiple = TRUE, Debug = Debug)
    })
  ...
  shiny::observeEvent(input$Pivot_Modal, {
    ...
    shiny::observeEvent(MeltData_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(MeltData_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(MeltData_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='MeltData_id.vars', Label='ID Columns', Choices = ChoiceList, SelectedDefault = if(length(input$MeltData_id.vars) > 0L) input$MeltData_id.vars else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='MeltData_measure.vars', Label='Measure Columns', Choices = ChoiceList, SelectedDefault = if(length(input$MeltData_measure.vars) > 0L) input$MeltData_measure.vars else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(CastData_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(CastData_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(CastData_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='CastData_id.vars', Label='LHS Columns', Choices = ChoiceList, SelectedDefault = if(length(input$CastData_id.vars) > 0L) input$CastData_id.vars else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='CastData_CastColumns', Label='RHS Columns', Choices = ChoiceList, SelectedDefault = if(length(input$CastData_CastColumns) > 0L) input$CastData_CastColumns else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='CastData_value.var', Label='Values Variable', Choices = ChoiceList, SelectedDefault = if(length(input$CastData_value.var) > 0L) input$CastData_value.var else NULL, Multiple = TRUE)
    })
  ...
  shiny::observeEvent(input$Columns_Modal, {
    ...
    shiny::observeEvent(TypeCast_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(TypeCast_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(TypeCast_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Numeric', Label='Convert Numeric', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Numeric) > 0L) input$TypeCast_Numeric else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Integer', Label='Convert Integer', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Integer) > 0L) input$TypeCast_Integer else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Character', Label='Convert Character', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Character) > 0L) input$TypeCast_Character else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Factor', Label='Convert Factor', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Factor) > 0L) input$TypeCast_Factor else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Logical', Label='Convert Logical', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Logical) > 0L) input$TypeCast_Logical else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Date', Label='Convert Date', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Date) > 0L) input$TypeCast_Date else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_ExcelDate', Label='Excel Date to R Date', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_ExcelDate) > 0L) input$TypeCast_ExcelDate else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TypeCast_Posix', Label='Convert Posix', Choices = ChoiceList, SelectedDefault = if(length(input$TypeCast_Posix) > 0L) input$TypeCast_Posix else NULL, Multiple = TRUE, Debug = Debug)
    })
    ...
    shiny::observeEvent(RenameColumns_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(RenameColumns_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(RenameColumns_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='RenameColumns_RenameColumn', Label='Select Column', Choices = ChoiceList, SelectedDefault = if(length(input$RenameColumns_RenameColumn) > 0L) input$RenameColumns_RenameColumn else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(TimeTrendColumn_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(TimeTrendColumn_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(TimeTrendColumn_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TimeTrendColumn_DateColumn', Label='Date Column', Choices = ChoiceList, SelectedDefault = if(length(input$TimeTrendColumn_DateColumn) > 0L) input$TimeTrendColumn_DateColumn else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TimeTrendColumn_GroupVars', Label='Group Variables', Choices = ChoiceList, SelectedDefault = if(length(input$TimeTrendColumn_GroupVars) > 0L) input$TimeTrendColumn_GroupVars else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(ConcatColumns_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(ConcatColumns_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(ConcatColumns_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='ConcatColumns', Label='Concat Columns', Choices = ChoiceList, SelectedDefault = if(length(input$ConcatColumns) > 0L) input$ConcatColumns else NULL, Multiple = TRUE)
    })
  })
  ...
  shiny::observeEvent(input$Misc_Modal, {
    ...
    shiny::observeEvent(TSF_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(TSF_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(TSF_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TSF_DateColumnName', Label='Date Column', Choices = ChoiceList, SelectedDefault = if(length(input$TSF_DateColumnName) > 0L) input$TSF_DateColumnName else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='TSF_GroupVariables', Label='Grouping Columns', Choices = ChoiceList, SelectedDefault = if(length(input$TSF_GroupVariables) > 0L) input$TSF_GroupVariables else NULL, Multiple = TRUE, Debug = Debug)
    })
    ...
    shiny::observeEvent(Roll_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(Roll_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(Roll_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Roll_RollVars', Label='Roll Vars', Choices = ChoiceList, SelectedDefault = if(length(input$Roll_RollVars) > 0L) input$Roll_RollVars else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Roll_NonRollVars', Label='Non Roll Vars', Choices = ChoiceList, SelectedDefault = if(length(input$Roll_NonRollVars) > 0L) input$Roll_NonRollVars else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Roll_DateColumnName', Label='Date Column', Choices = ChoiceList, SelectedDefault = if(length(input$Roll_DateColumnName) > 0L) input$Roll_DateColumnName else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Roll_GroupVariables', Label='Grouping Columns', Choices = ChoiceList, SelectedDefault = if(length(input$Roll_GroupVariables) > 0L) input$Roll_GroupVariables else NULL, Multiple = TRUE, Debug = Debug)
    })
  })
  ...
  shiny::observeEvent(input$Windowing_Modal, {
    ...
    shiny::observeEvent(AutoLagRollStats_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoLagRollStats_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoLagRollStats_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoLagRollStats_Targets', Label='Target Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoLagRollStats_Targets) > 0L) input$AutoLagRollStats_Targets else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoLagRollStats_GroupVars', Label='By-Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoLagRollStats_GroupVars) > 0L) input$AutoLagRollStats_GroupVars else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoLagRollStats_DateColumn', Label='Date Variable', Choices = ChoiceList, SelectedDefault = if(length(input$AutoLagRollStats_DateColumn) > 0L) input$AutoLagRollStats_DateColumn else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(AutoDiffLagN_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoDiffLagN_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoDiffLagN_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDiffLagN_DateVariable', Label='Date Variable', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDiffLagN_DateVariable) > 0L) input$AutoDiffLagN_DateVariable else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDiffLagN_GroupVariables', Label='By-Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDiffLagN_GroupVariables) > 0L) input$AutoDiffLagN_GroupVariables else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDiffLagN_DiffVariables', Label='Numeric Diff Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDiffLagN_DiffVariables) > 0L) input$AutoDiffLagN_DiffVariables else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDiffLagN_DiffDateVariables', Label='Date Diff Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDiffLagN_DiffDateVariables) > 0L) input$AutoDiffLagN_DiffDateVariables else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoDiffLagN_DiffGroupVariables', Label='Group Diff Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoDiffLagN_DiffGroupVariables) > 0L) input$AutoDiffLagN_DiffGroupVariables else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(AutoLagRollMode_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoLagRollMode_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoLagRollMode_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoLagRollMode_Targets', Label='Target Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoLagRollMode_Targets) > 0L) input$AutoLagRollMode_Targets else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoLagRollMode_GroupingVars', Label='By-Variables', Choices = ChoiceList, SelectedDefault = if(length(input$AutoLagRollMode_GroupingVars) > 0L) input$AutoLagRollMode_GroupingVars else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoLagRollMode_SortDateName', Label='Date Variable', Choices = ChoiceList, SelectedDefault = if(length(input$AutoLagRollMode_SortDateName) > 0L) input$AutoLagRollMode_SortDateName else NULL, Multiple = TRUE)
    })
  ...
  shiny::observeEvent(input$Calendar_Modal, {
    ...
    shiny::observeEvent(CalendarVariables_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(CalendarVariables_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(CalendarVariables_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='CalendarVariables_DateVariables', Label='Date Columns', Choices = ChoiceList, SelectedDefault = if(length(input$CalendarVariables_DateVariables) > 0L) input$CalendarVariables_DateVariables else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(HolidayVariables_DataReactive(), {
      ...
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(bla))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(bla, Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID=   'HolidayVariables_DateVariables', Label='Date Variables', Choices = ChoiceList, SelectedDefault = if(length(input$HolidayVariables_DateVariables) > 0L) input$HolidayVariables_DateVariables else NULL, Multiple = TRUE)
    })
  })
  ...
  shiny::observeEvent(input$Categorical_Modal, {
    DataMuse:::Categorical_Modal_Fun(
      id = 'CategoricalFEID',
    ...
    shiny::observeEvent(CategoricalEncoding_TrainDataReactive(), {
      ...
      if(length(nam) > 0L) {
        ChoiceList <- list()
        ColTypes <- unique(DataMuse:::ColTypes(nam))
        for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(nam, Types = ColTypes[i])
        DataMuse:::PickerInput(session = session, Update = TRUE, InputID='CategoricalEncoding_GroupVariables', Label='Group Variables', Choices = ChoiceList, SelectedDefault = if(length(input$CategoricalEncoding_GroupVariables) > 0L) input$CategoricalEncoding_GroupVariables else NULL, Multiple = TRUE)
        DataMuse:::PickerInput(session = session, Update = TRUE, InputID='CategoricalEncoding_TargetVariable', Label='Target Columns', Choices = ChoiceList, SelectedDefault = if(length(input$CategoricalEncoding_TargetVariable) > 0L) input$CategoricalEncoding_TargetVariable else NULL, Multiple = TRUE)
      }
    })
    ...
    shiny::observeEvent(DummifyDT_SelectData_Reactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(DummifyDT_SelectData_Reactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(DummifyDT_SelectData_Reactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='DummifyDT_Cols', Label='Target Columns', Choices = ChoiceList, SelectedDefault = if(length(input$DummifyDT_Cols) > 0L) input$DummifyDT_Cols else NULL, Multiple = TRUE)
    })
  })
  ...
  shiny::observeEvent(input$Numeric_Modal, {
    ...
    shiny::observeEvent(PercentRank_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(PercentRank_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(PercentRank_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='PercentRank_ColNames', Label='Target Columns', Choices = ChoiceList, SelectedDefault = if(length(input$PercentRank_ColNames) > 0L) input$PercentRank_ColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='PercentRank_GroupVars', Label='By-Variables', Choices = ChoiceList, SelectedDefault = if(length(input$PercentRank_GroupVars) > 0L) input$PercentRank_GroupVars else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(PercentRank_DataReactive(), {
      ...
      ColTypes <- unique(DataMuse:::ColTypes(PercentRank_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(PercentRank_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Standardize_ColNames', Label='Target Columns', Choices = ChoiceList, SelectedDefault = if(length(input$Standardize_ColNames) > 0L) input$Standardize_ColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Standardize_GroupVars', Label='By-Variables', Choices = ChoiceList, SelectedDefault = if(length(input$Standardize_GroupVars) > 0L) input$Standardize_GroupVars else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(AutoInteraction_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoInteraction_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoInteraction_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoInteraction_NumericVars', Label='Target Columns', Choices = ChoiceList, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(AutoTransformationCreate_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoTransformationCreate_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoTransformationCreate_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='AutoTransformationCreate_ColumnNames', Label='Target Columns', Choices = ChoiceList, SelectedDefault = if(length(input$AutoTransformationCreate_ColumnNames) > 0L) input$AutoTransformationCreate_ColumnNames else NULL, Multiple = TRUE)
    })
  ...
  shiny::observeEvent(input$AnomalyDetection_Modal, {
    DataMuse:::AnomalyDetection_Modal_Fun(
    ...
    shiny::observeEvent(IsolationForest_H2O_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(IsolationForest_H2O_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(IsolationForest_H2O_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'IsolationForest_H2O_Features', Label = 'Features', Choices = ChoiceList, SelectedDefault = if(length(input$IsolationForest_H2O_Features) > 0L) input$IsolationForest_H2O_Features else NULL, Multiple = TRUE, Debug = Debug)
    })
  })
  ...
  shiny::observeEvent(input$DimensionalityReduction_Modal, {
    DataMuse:::DimensionalityReduction_Modal_Fun(
      id = 'DimReductionFEID',
    ...
    shiny::observeEvent(AutoEncoder_H2O_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(AutoEncoder_H2O_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(AutoEncoder_H2O_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'AutoEncoder_H2O_Features', Label = 'Features', Choices = ChoiceList, SelectedDefault = if(length(input$AutoEncoder_H2O_Features) > 0L) input$AutoEncoder_H2O_Features else NULL, Multiple = TRUE, Debug = Debug)
    })
  })
  ...
  shiny::observeEvent(input$Clustering_Modal, {
    ...
    shiny::observeEvent(Kmeans_H2O_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(Kmeans_H2O_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(Kmeans_H2O_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'Kmeans_H2O_Features', Label = 'Features', Choices = ChoiceList, SelectedDefault = if(length(input$Kmeans_H2O_Features) > 0L) input$Kmeans_H2O_Features else NULL, Multiple = TRUE)
    })
  })
  ...
  shiny::observeEvent(input$NLP_Modal, {
    ...
    shiny::observeEvent(Word2Vec_H2O_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(Word2Vec_H2O_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(Word2Vec_H2O_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID='Word2Vec_H2O_stringCol', Label='Text Variables', Choices = ChoiceList, SelectedDefault = if(length(input$Word2Vec_H2O_stringCol) > 0L) input$Word2Vec_H2O_stringCol else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(TextSummary_SelectDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(TextSummary_SelectDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(TextSummary_SelectDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'TextSummary_TextColumns', Label = 'Text Columns', Choices = ChoiceList, SelectedDefault = if(length(input$TextSummary_TextColumns) > 0L) input$TextSummary_TextColumns else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(Sentiment_SelectDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(Sentiment_SelectDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(Sentiment_SelectDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'Sentiment_TextColumns', Label = 'Text Columns', Choices = ChoiceList, SelectedDefault = if(length(input$Sentiment_TextColumns) > 0L) input$Sentiment_TextColumns else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'Sentiment_CombineTextGroupVar', Label = 'Combine Text Group Variable', Choices = ChoiceList, SelectedDefault = if(length(input$Sentiment_CombineTextGroupVar) > 0L) input$Sentiment_CombineTextGroupVar else NULL, Multiple = TRUE, Debug = Debug)
    })
    ...
    shiny::observeEvent(Readability_SelectDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(Readability_SelectDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(Readability_SelectDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'Readability_TextColumns', Label = 'Text Columns', Choices = ChoiceList, SelectedDefault = NULL, SelectedText = if(length(input$Readability_TextColumns) > 0L) input$Readability_TextColumns else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(LexicalDiversity_SelectDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(LexicalDiversity_SelectDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(LexicalDiversity_SelectDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = 'LexicalDiversity_TextColumns', Label = 'Text Columns', Choices = ChoiceList, SelectedDefault = if(length(input$LexicalDiversity_TextColumns) > 0L) input$LexicalDiversity_TextColumns else NULL, Multiple = TRUE, Debug = Debug)
    })
  })
  ...
  shiny::observeEvent(input$CatBoost_Modal, {
    ...
    shiny::observeEvent(shiny::req(CatBoost_dataReactive()), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(CatBoost_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(CatBoost_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CatBoost_TargetColumnName', Label='Select Target', Choices = ChoiceList, SelectedDefault = if(length(input$CatBoost_TargetColumnName) > 0L) input$CatBoost_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CatBoost_FeatureColNames', Label='Select Features', Choices = ChoiceList, SelectedDefault = if(length(input$CatBoost_FeatureColNames) > 0L) input$CatBoost_FeatureColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CatBoost_PrimaryDateColumn', Label='Select Date', Choices = ChoiceList, SelectedDefault = if(length(input$CatBoost_PrimaryDateColumn) > 0L) input$CatBoost_PrimaryDateColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CatBoost_WeightsColumnName', Label='Weights Column', Choices = ChoiceList, SelectedDefault = if(length(input$CatBoost_WeightsColumnName) > 0L) input$CatBoost_WeightsColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CatBoost_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, SelectedDefault = if(length(input$CatBoost_TransformNumericColumns) > 0L) input$CatBoost_TransformNumericColumns else NULL, Multiple = TRUE)
    })
    ...
    shiny::observeEvent(CatBoost_TargetTypeReactive(), {
      ...
      out1 <- DataMuse:::Shiny.ML.CatBoost.GridEvalMetricsOptions(CatBoost_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='CatBoost_grid_eval_metric', Label='Grid Tuning Evaluation Metric', Choices = out1$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out1$Default, CloseAfterSelect = FALSE)
      out2 <- DataMuse:::Shiny.ML.CatBoost.LossFunctionOptions(CatBoost_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='CatBoost_LossFunction', Label='Loss Function', Choices = out2$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out2$Default, CloseAfterSelect = FALSE)
      out3 <- DataMuse:::Shiny.ML.CatBoost.EvalMetricOptions(CatBoost_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='CatBoost_EvalMetric', Label='Evaluation Metric', Choices = out3$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out3$Default, CloseAfterSelect = FALSE)
    })
  })
  ...
  shiny::observeEvent(input$XGBoost_Modal, {
    ...
    shiny::observeEvent(XGBoost_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(XGBoost_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(XGBoost_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='XGBoost_TargetColumnName', Label='Select Target', Choices = ChoiceList, SelectedDefault = if(length(input$XGBoost_TargetColumnName) > 0L) input$XGBoost_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='XGBoost_FeatureColNames', Label='Select Features', Choices = ChoiceList, SelectedDefault = if(length(input$XGBoost_FeatureColNames) > 0L) input$XGBoost_FeatureColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='XGBoost_PrimaryDateColumn', Label='Select Date', Choices = ChoiceList, SelectedDefault = if(length(input$XGBoost_PrimaryDateColumn) > 0L) input$XGBoost_PrimaryDateColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='XGBoost_WeightsColumnName', Label='Weights Column', Choices = ChoiceList, SelectedDefault = if(length(input$XGBoost_WeightsColumnName) > 0L) input$XGBoost_WeightsColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='XGBoost_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, SelectedDefault = if(length(input$XGBoost_TransformNumericColumns) > 0L) input$XGBoost_TransformNumericColumns else NULL, Multiple = TRUE)
    })

    XGBoost_TargetTypeReactive <- shiny::reactive({input$XGBoost_TargetType})
    shiny::observeEvent(XGBoost_TargetTypeReactive(), {
      out <- DataMuse:::Shiny.ML.CatBoost.GridEvalMetricsOptions(XGBoost_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='XGBoost_grid_eval_metric', Label='Grid Tuning Evaluation Metric', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
      out <- DataMuse:::Shiny.ML.XGBoost.LossFunctionOptions(XGBoost_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='XGBoost_LossFunction', Label='Loss Function', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
      out <- DataMuse:::Shiny.ML.XGBoost.EvalMetricOptions(XGBoost_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='XGBoost_EvalMetric', Label='Evaluation Metric', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
    })
  })
  ...
  shiny::observeEvent(input$LightGBM_Modal, {
    ...
    shiny::observeEvent(LightGBM_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(LightGBM_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(LightGBM_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='LightGBM_TargetColumnName', Label='Select Target', Choices = ChoiceList, SelectedDefault = if(length(input$LightGBM_TargetColumnName) > 0L) input$LightGBM_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='LightGBM_FeatureColNames', Label='Select Features', Choices = ChoiceList, SelectedDefault = if(length(input$LightGBM_FeatureColNames) > 0L) input$LightGBM_FeatureColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='LightGBM_PrimaryDateColumn', Label='Select Date', Choices = ChoiceList, SelectedDefault = if(length(input$LightGBM_PrimaryDateColumn) > 0L) input$LightGBM_PrimaryDateColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='LightGBM_WeightsColumnName', Label='Weights Column', Choices = ChoiceList, SelectedDefault = if(length(input$LightGBM_WeightsColumnName) > 0L) input$LightGBM_WeightsColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='LightGBM_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, SelectedDefault = if(length(input$LightGBM_TransformNumericColumns) > 0L) input$LightGBM_TransformNumericColumns else NULL, Multiple = TRUE)
    })

    LightGBM_TargetTypeReactive <- shiny::reactive({input$LightGBM_TargetType})
    shiny::observeEvent(LightGBM_TargetTypeReactive(), {
      out <- DataMuse:::Shiny.ML.CatBoost.GridEvalMetricsOptions(LightGBM_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='LightGBM_grid_eval_metric', Label='Grid Tuning Evaluation Metric', Choices = tryCatch({out$Choices}, error = function(x) NULL), Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
      out <- DataMuse:::Shiny.ML.LightGBM.LossFunctionOptions(LightGBM_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='LightGBM_metric', Label='Loss Function', Choices = tryCatch({out$Choices}, error = function(x) NULL), Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
    })
  })
  ...
  shiny::observeEvent(input$H2O_DRF_Modal, {
    ...
    shiny::observeEvent(H2O_DRF_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(H2O_DRF_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(H2O_DRF_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_DRF_TargetColumnName', Label='Select Target', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_DRF_TargetColumnName) > 0L) input$H2O_DRF_TargetColumnName else NULL,  Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_DRF_FeatureColNames', Label='Select Features', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_DRF_FeatureColNames) > 0L) input$H2O_DRF_FeatureColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_DRF_PrimaryDateColumn', Label='Select Date', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_DRF_PrimaryDateColumn) > 0L) input$H2O_DRF_PrimaryDateColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_DRF_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_DRF_TransformNumericColumns) > 0L) input$H2O_DRF_TransformNumericColumns else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_DRF_WeightsColumn', Label='Weights Column', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_DRF_WeightsColumn) > 0L) input$H2O_DRF_WeightsColumn else NULL, Multiple = TRUE, MaxVars = 1L)
    })

    H2O_DRF_TargetTypeReactive <- shiny::reactive({input$H2O_DRF_TargetType})
    shiny::observeEvent(H2O_DRF_TargetTypeReactive(), {
      out <- DataMuse:::Shiny.ML.H2O_DRF.EvalMetricOptions(shiny::req(H2O_DRF_TargetTypeReactive()))
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='H2O_DRF_eval_metric', Label='Loss Function', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
    })
  ...
  shiny::observeEvent(input$H2O_GBM_Modal, {
    ...
    shiny::observeEvent(H2O_GBM_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(H2O_GBM_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(H2O_GBM_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_GBM_TargetColumnName', Label='Select Target', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GBM_TargetColumnName) > 0L) input$H2O_GBM_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_GBM_FeatureColNames', Label='Select Features', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GBM_FeatureColNames) > 0L) input$H2O_GBM_FeatureColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_GBM_PrimaryDateColumn', Label='Select Date', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GBM_PrimaryDateColumn) > 0L) input$H2O_GBM_PrimaryDateColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_GBM_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GBM_TransformNumericColumns) > 0L) input$H2O_GBM_TransformNumericColumns else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = 'H2O_GBM_WeightsColumn', Label='Weights Column', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GBM_WeightsColumn) > 0L) input$H2O_GBM_WeightsColumn else NULL, Multiple = TRUE, MaxVars = 1L)
    })

    H2O_GBM_TargetTypeReactive <- shiny::reactive({input$H2O_GBM_TargetType})
    shiny::observeEvent(H2O_GBM_TargetTypeReactive(), {
      out <- DataMuse:::Shiny.ML.H2O.DistributionOptions(shiny::req(H2O_GBM_TargetTypeReactive()))
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='H2O_GBM_Distribution', Label='Loss Function', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
    })
  ...
  shiny::observeEvent(input$H2O_GLM_Modal, {
    ...
    shiny::observeEvent(H2O_GLM_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(H2O_GLM_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(H2O_GLM_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_TargetColumnName', Label='Select Target', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GLM_TargetColumnName) > 0L) input$H2O_GLM_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_FeatureColNames', Label='Select Features', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GLM_FeatureColNames) > 0L) input$H2O_GLM_FeatureColNames else NULL, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_PrimaryDateColumn', Label='Select Date', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GLM_PrimaryDateColumn) > 0L) input$H2O_GLM_PrimaryDateColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_WeightsColumn', Label='Weights Column', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GLM_WeightsColumn) > 0L) input$H2O_GLM_WeightsColumn else NULL, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, SelectedDefault = if(length(input$H2O_GLM_TransformNumericColumns) > 0L) input$H2O_GLM_TransformNumericColumns else NULL, Multiple = TRUE)
    })

    H2O_GLM_TargetTypeReactive <- shiny::reactive({input$H2O_GLM_TargetType})
    shiny::observeEvent(H2O_GLM_TargetTypeReactive(), {
      out <- DataMuse:::Shiny.ML.H2O.GLM.DistributionOptions(H2O_GLM_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_Distribution', Label='Distribution', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = 'AUTO', CloseAfterSelect = FALSE)
      out <- DataMuse:::Shiny.ML.H2O.LinkOptions(H2O_GLM_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_Link', Label='Link', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = 'AUTO', CloseAfterSelect = FALSE)
      out <- DataMuse:::Shiny.ML.H2O_GLM.EvalMetricOptions(H2O_GLM_TargetTypeReactive())
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='H2O_GLM_eval_metric', Label='Eval Metric', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
    })
  })
  ...
  shiny::observeEvent(input$H2O_HGLM_Modal, {
    ...
    shiny::observeEvent(H2O_HGLM_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(H2O_HGLM_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(H2O_HGLM_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_HGLM_TargetColumnName', Label='Select Target', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_HGLM_FeatureColNames', Label='Select Features', Choices = ChoiceList, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_HGLM_RandomColNumbers', Label='Random Effects', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_HGLM_WeightsColumn', Label='Weights Column', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='H2O_HGLM_TransformNumericColumns', Label='Transform Columns', Choices = ChoiceList, Multiple = TRUE)
    })

    H2O_HGLM_TargetTypeReactive <- shiny::reactive({input$H2O_GLM_TargetType})
    shiny::observeEvent(H2O_HGLM_TargetTypeReactive(), {
      out <- DataMuse:::Shiny.ML.H2O_GLM.EvalMetricOptions(H2O_HGLM_TargetTypeReactive())
      if(Debug) {print(out); print(H2O_HGLM_TargetTypeReactive())}
      DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID='H2O_HGLM_eval_metric', Label='Eval Metric', Choices = out$Choices, Multiple = TRUE, MaxVars = 1, SelectedDefault = out$Default, CloseAfterSelect = FALSE)
    })
  ...
  shiny::observeEvent(input$CausalMediation_Modal, {
    DataMuse:::CausalMediation_Modal_Fun(
    ...
    shiny::observeEvent(CausalMediation_dataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(CausalMediation_dataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(CausalMediation_dataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_OutcomeTargetVariable', Label='Outcome Target', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_TreatmentVariable', Label='Treatment Variable', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_MediatorVariable', Label='Mediator Variable', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediationList_MediatorVariable', Label='Mediator Variable', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_SurvivalEventVariable', Label='Survival Event Variable', Choices = ChoiceList, Multiple = TRUE, MaxVars = 1L)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_Covariates', Label='Outcome Covariates', Choices = ChoiceList, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_OM_MediatorCovariates', Label='Outcome Mediator Covariates', Choices = ChoiceList, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_OM_TreatmentCovariates', Label='Outcome Treatment Covariates', Choices = ChoiceList, Multiple = TRUE)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID='CausalMediation_MM_TreatmentCovariates', Label='Mediation Treatment Covariates', Choices = ChoiceList, Multiple = TRUE)
    }, ignoreInit = TRUE)
  ...
  shiny::observeEvent(input$TBATS_Modal, {
    ...
    shiny::observeEvent(TBATS_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(TBATS_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(TBATS_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "TBATS_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$TBATS_TargetColumnName) > 0L) input$TBATS_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "TBATS_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$TBATS_DateColumnName) > 0L) input$TBATS_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$SARIMA_Modal, {
    ...
    shiny::observeEvent(SARIMA_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(SARIMA_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(SARIMA_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "SARIMA_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$SARIMA_TargetColumnName) > 0L) input$SARIMA_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "SARIMA_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$SARIMA_DateColumnName) > 0L) input$SARIMA_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$ETS_Modal, {
    ...
    shiny::observeEvent(ETS_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(ETS_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(ETS_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "ETS_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$ETS_TargetColumnName) > 0L) input$ETS_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "ETS_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$ETS_DateColumnName) > 0L) input$ETS_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$ARFIMA_Modal, {
    ...
    shiny::observeEvent(ARFIMA_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(ARFIMA_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(ARFIMA_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "ARFIMA_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$ARFIMA_TargetColumnName) > 0L) input$ARFIMA_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "ARFIMA_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$ARFIMA_DateColumnName) > 0L) input$ARFIMA_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$NNET_Modal, {
    ...
    shiny::observeEvent(NNET_DataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(NNET_DataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(NNET_DataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "NNET_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$NNET_TargetColumnName) > 0L) input$NNET_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "NNET_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$NNET_DateColumnName) > 0L) input$NNET_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$CatBoostCARMA_Modal, {
    DataMuse:::CatBoostCARMA_Modal_Fun(
    ...
    shiny::observeEvent(CatBoostCARMA_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(CatBoostCARMA_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(CatBoostCARMA_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "CatBoostCARMA_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$CatBoostCARMA_TargetColumnName) > 0L) input$CatBoostCARMA_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "CatBoostCARMA_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$CatBoostCARMA_DateColumnName) > 0L) input$CatBoostCARMA_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = "CatBoostCARMA_GroupVariables", Label = "Group Variables", Choices = ChoiceList, SelectedDefault = if(length(input$CatBoostCARMA_GroupVariables) > 0L) input$CatBoostCARMA_GroupVariables else NULL, Multiple = TRUE)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$XGBoostCARMA_Modal, {
    DataMuse:::XGBoostCARMA_Modal_Fun(
      id = 'XGBoostCARMAMLID',
    ...
    shiny::observeEvent(XGBoostCARMA_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(XGBoostCARMA_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(XGBoostCARMA_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = "XGBoostCARMA_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$XGBoostCARMA_TargetColumnName) > 0L) input$XGBoostCARMA_TargetColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = "XGBoostCARMA_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$XGBoostCARMA_DateColumnName) > 0L) input$XGBoostCARMA_DateColumnName else NULL, Multiple = TRUE, MaxVars = 1)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = "XGBoostCARMA_GroupVariables", Label = "Group Variables", Choices = ChoiceList, SelectedDefault = if(length(input$XGBoostCARMA_GroupVariables) > 0L) input$XGBoostCARMA_GroupVariables else NULL, Multiple = TRUE)
    }, ignoreInit = FALSE)
  })
  ...
  shiny::observeEvent(input$LightGBMCARMA_Modal, {
    DataMuse:::LightGBMCARMA_Modal_Fun(
    ...
    shiny::observeEvent(LightGBMCARMA_TrainDataReactive(), {
      ChoiceList <- list()
      ColTypes <- unique(DataMuse:::ColTypes(LightGBMCARMA_TrainDataReactive()))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(LightGBMCARMA_TrainDataReactive(), Types = ColTypes[i])
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = "LightGBMCARMA_TargetColumnName", Label = "Target Variable", Choices = ChoiceList, SelectedDefault = if(length(input$LightGBMCARMA_TargetColumnName) > 0L) input$LightGBMCARMA_TargetColumnName else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = "LightGBMCARMA_DateColumnName", Label = "Date Variable", Choices = ChoiceList, SelectedDefault = if(length(input$LightGBMCARMA_DateColumnName) > 0L) input$LightGBMCARMA_DateColumnName else NULL, Multiple = TRUE, Debug = Debug)
      DataMuse:::PickerInput(session = session, Update = TRUE, InputID = "LightGBMCARMA_GroupVariables", Label = "Group Variables", Choices = ChoiceList, SelectedDefault = if(length(input$LightGBMCARMA_GroupVariables) > 0L) input$LightGBMCARMA_GroupVariables else NULL, Multiple = TRUE, Debug = Debug)
    }, ignoreInit = FALSE)
  ...
  shiny::observeEvent(input$tabss, {
    ...
    if(!is.na(PMisc) && PMisc > 0L) {
      if(tryCatch({input$tabss}, error = function(x) "aa") %in% paste0('Plots ', PMisc) && PMisc > 0L) {
        ...
        out <- DataMuse:::PlotSize_(input, PMisc, 1, 1)
        ...
        if(PlotOutput_Start1) {
          ...
          plot_output_list[[PMisc]] <- echarts4r::renderEcharts4r({
            DataMuse:::BlankEchart(Type = "Poincare", Theme = tryCatch({EchartsTheme}, error = function(x) "macarons"))
          })
  ...
  shiny::observeEvent({eval(MLModelSelectionExpression)}, {
    ...
    MLModelSelection <- DataMuse::ReturnParam(xx = tryCatch({input[[paste0("MLReportsModelSelection", Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)
    ...
    if(data.table::is.data.table(d)) {
      ColTypes <- unique(DataMuse:::ColTypes(d))
      for(i in seq_along(ColTypes)) ChoiceList[[ColTypes[i]]] <- DataMuse:::ColNameFilter(d, Types = ColTypes[i])
  ...
  shiny::observeEvent({eval(EDASelectionExpression)}, {
    Page <- tryCatch({as.integer(gsub("[^\\d]+", "", input$tabss, perl=TRUE))}, error = function(x) 0L)
    EDAData <- DataMuse::ReturnParam(xx = tryCatch({input[[paste0("EDAData", Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)
    ChoiceList <- list()
  ...
  lapply(OKModalsButtons, FUN = function(x) {
    shiny::observeEvent(input[[x]], {
      ...
      PlotType <- PlotMap[PlotNumber == eval(PlotNums)][["PlotType"]]#DataMuse::ReturnParam(xx = input[[paste0('PlotID_',PlotNums)]], Type = "character", Default = NULL)#tryCatch({input[[paste0('Plot',shiny::req(PN()))]]}, error = function(x) NULL)
      PlotType <<- PlotType
    ...
    shiny::observeEvent(input[[x]], {
      ...
      if(length(PlotType) > 0L && PlotType != "None") {
        ...
        if(PlotType %in% "CorrelogramPlot") {
          ...
          DataMuse:::Plots.3D.Target.NoGroup.Modal(id = paste0('Eval.PDHeatMap',PlotNums,PlotType), PlotNumber = PlotNums)
        } else if(PlotType %in% c("HistogramPlot","DensityPlot")) {
          DataMuse:::Plots.Distribution.Modal(id = paste0('Distribution',PlotNums,PlotType), PlotNumber = PlotNums)
          ...
          DataMuse:::Plots.3D.Modal.NoFacet(id = paste0('ThreeD',PlotNums,PlotType), PlotNumber = PlotNums)
        } else if(PlotType %in% c("CalibrationLine","ConfusionMatrixHeatmap","ROCPlot","LiftPlot","GainsPlot","Residuals","ResidScatter")) {
          DataMuse:::Plots.Eval.ZY.Modal(id = paste0('Eval.ZY',PlotNums,PlotType), PlotNumber = PlotNums)
        } else if(PlotType %in% c("CalibrationBox")) {
          DataMuse:::Plots.Eval.ZY.NoFacet.Modal(id = paste0('Eval.ZY.NoFacet',PlotNums,PlotType), PlotNumber = PlotNums)
        } else if(PlotType %in% c("PartialDependenceBox")) {
          DataMuse:::Plots.Eval.XYZ.NoGroup.Modal(id = paste0('Eval.XYZ.NoGroup',PlotNums,PlotType), PlotNumber = PlotNums)
        } else if(PlotType %in% "VariableImportance") {
          DataMuse:::Plots.Eval.VarImp.Modal(id = paste0('EvalVarImp',PlotNums,PlotType), PlotNumber = PlotNums)
        } else if(PlotType %in% "ShapleyImportance") {
          DataMuse:::Plots.Eval.ShapImp.Modal(id = paste0('EvalShapImp',PlotNums,PlotType), PlotNumber = PlotNums)
        } else {
          DataMuse:::Plots.Eval.Modal(id = paste0('Eval',PlotNums,PlotType), PlotNumber = PlotNums)
        }

        DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('Plot',PlotNums,'_SelectData'), Label = 'Choose data set', Choices = names(DataList), Multiple = TRUE, MaxVars = 100, SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)

        # Experimenting with this code here as well as below
        if(length(PlotType) > 0L && !PlotType %in% paste0("Plot", 1L:36L)) {
          key <- paste0(PlotType, PlotNums)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('YVarTrans',PlotNums), Label = 'Y-Variable Transformation', Choices = c("Identity","Asinh","Log","LogPlus1","Sqrt","Asin","Logit","BoxCox","YeoJohnson"), Multiple = TRUE, MaxVars = 1, SelectedDefault = "Identity", PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('DualYVarTrans',PlotNums), Label = 'Dual Axis Y-Variable Transformation', Choices = c("Identity","Asinh","Log","LogPlus1","Sqrt","Asin","Logit","BoxCox","YeoJohnson"), Multiple = TRUE, MaxVars = 1, SelectedDefault = "Identity", PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('XVarTrans',PlotNums), Label = 'X-Variable Transformation', Choices = c("Identity","Asinh","Log","LogPlus1","Sqrt","Asin","Logit","BoxCox","YeoJohnson"), Multiple = TRUE, MaxVars = 1, SelectedDefault = "Identity", PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('ZVarTrans',PlotNums), Label = 'Z-Variable Transformation', Choices = c("Identity","Asinh","Log","LogPlus1","Sqrt","Asin","Logit","BoxCox","YeoJohnson"), Multiple = TRUE, MaxVars = 1, SelectedDefault = "Identity", PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('PlotDateAgg',PlotNums), Label = 'Date Aggregration Level', Choices = c("as-is","second","minute","hour","day","week","month","quarter","year"), Multiple = TRUE, MaxVars = 1, SelectedDefault = "as-is", PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('AggMethod',PlotNums), Choices = c('count','proportion','mean','meanabs','median','medianabs','sum','sumabs','sd','sdabs', 'skewness','skewnessabs', 'kurtosis','kurtosisabs','CoeffVar','CoeffVarabs'), SelectedDefault = 'mean', Label = "Aggregation Method", PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SliderInput(session = session, Update = TRUE, input=input, InputID = paste0('FacetRows',PlotNums), Label = "Facet Rows", PlotDropDown = PlotDropDown, Key = key, Step = 1, Value = 1, Min = 1, Max = 12)
          DataMuse:::SliderInput(session = session, Update = TRUE, input=input, InputID = paste0('FacetCols',PlotNums), Label = "Facet Columns", PlotDropDown = PlotDropDown, Key = key, Step = 1, Value = 1, Min = 1, Max = 12)
          DataMuse:::TextInput(session = session, Update = TRUE, input=input, InputID = paste0('Title', PlotNums), Label='Rename Title', Value = NULL, PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('ShowLabels', PlotNums), Label='Show Labels', Choices = c(TRUE,FALSE), Multiple=TRUE, MaxVars = 1, SelectedDefault = FALSE, CloseAfterSelect = TRUE, PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::TextInput(session = session, Update = TRUE, input=input, InputID = paste0("YAxisTitle", PlotNums), Label = "Y-Axis Title ('None' for blank)", Value = NULL, PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::TextInput(session = session, Update = TRUE, input=input, InputID = paste0("XAxisTitle", PlotNums), Label = "X-Axis Title ('None' for blank)", Value = NULL, PlotDropDown = PlotDropDown, Key = key)
          DataMuse:::NumericInput(session = session, Update = TRUE, input=input, InputID = paste0("PlottingMaxLags", PlotNums), Label = 'Max Lags', Value = 50, Min = 5, Max = 200, Step = 1)
          DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("PlottingTimeUnit", PlotNums), Label = "Time Agg Level", Choices = c("hour", "day", "week", "month", "quarter", "year"), Multiple = TRUE, MaxVars = 1)
          ...
          shiny::observeEvent(data(), {
            ...
            if(length(data()) > 0L) {
              Coltypes <- unique(DataMuse:::ColTypes(data()))
              for(i in seq_along(Coltypes)) {
                ChoiceList[[Coltypes[i]]] <- DataMuse:::ColNameFilter(data(), Types = Coltypes[i])
                ChoiceListSelectize[[Coltypes[i]]] <- as.list(DataMuse:::ColNameFilter(data(), Types = Coltypes[i]))
              }
            }

            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('YVar',shiny::isolate(PlotNums)), Label = 'Y-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = if(length(input[[paste0('YVar',shiny::isolate(PlotNums))]]) > 0L) input[[paste0('YVar',shiny::isolate(PlotNums))]] else NULL)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('DualYVar',shiny::isolate(PlotNums)), Label = 'Dual Axis Y-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = if(length(input[[paste0('DualYVar',shiny::isolate(PlotNums))]]) > 0L) input[[paste0('DualYVar',shiny::isolate(PlotNums))]] else NULL)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('XVar',shiny::isolate(PlotNums)), Label = 'X-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = if(length(input[[paste0('XVar',shiny::isolate(PlotNums))]]) > 0L) input[[paste0('XVar',shiny::isolate(PlotNums))]] else NULL)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('ZVar',shiny::isolate(PlotNums)), Label = 'Z-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = if(length(input[[paste0('ZVar',shiny::isolate(PlotNums))]]) > 0L) input[[paste0('ZVar',shiny::isolate(PlotNums))]] else NULL)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('PlottingDateVar',shiny::isolate(PlotNums)), Label = 'Date Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = if(length(input[[paste0('PlottingDateVar',shiny::isolate(PlotNums))]]) > 0L) input[[paste0('PlottingDateVar',shiny::isolate(PlotNums))]] else NULL)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('GroupVars',shiny::isolate(PlotNums)), Label='Select Group Variables', Choices=ChoiceList, Multiple=TRUE, MaxVars = 3L, SelectedDefault = if(length(input[[paste0('GroupVars',shiny::isolate(PlotNums))]]) > 0L) input[[paste0('GroupVars',shiny::isolate(PlotNums))]] else NULL)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',shiny::isolate(PlotNums),'_1'), Label = 'Filter Variable 1', Choices = ChoiceList, SelectedDefault = if(length(input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_1')]]) > 0L) input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_1')]] else NULL, Multiple = TRUE, MaxVars = 2L)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',shiny::isolate(PlotNums),'_2'), Label = 'Filter Variable 2', Choices = ChoiceList, SelectedDefault = if(length(input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_2')]]) > 0L) input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_2')]] else NULL, Multiple = TRUE, MaxVars = 2L)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',shiny::isolate(PlotNums),'_3'), Label = 'Filter Variable 3', Choices = ChoiceList, SelectedDefault = if(length(input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_3')]]) > 0L) input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_3')]] else NULL, Multiple = TRUE, MaxVars = 2L)
            DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',shiny::isolate(PlotNums),'_4'), Label = 'Filter Variable 4', Choices = ChoiceList, SelectedDefault = if(length(input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_4')]]) > 0L) input[[paste0('FilterVariable_',shiny::isolate(PlotNums),'_4')]] else NULL, Multiple = TRUE, MaxVars = 2L)
            DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',shiny::isolate(PlotNums),'_1'), SelectedDefault = DataMuse:::FL_Default(tryCatch({shiny::isolate(data())}, error = function(x) NULL), x=tryCatch({FilterVariable_1_1()}, error = function(x) NULL)), Label = 'Logical Operation', Choices = c('<','>','<=','>=','%in%','%like%','%between%','not %between%'), Multiple = TRUE, MaxVars = 1, PlotDropDown = PlotDropDown, Key = key)
            DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',shiny::isolate(PlotNums),'_2'), SelectedDefault = DataMuse:::FL_Default(tryCatch({shiny::isolate(data())}, error = function(x) NULL), x=tryCatch({FilterVariable_1_2()}, error = function(x) NULL)), Label = 'Logical Operation', Choices = c('<','>','<=','>=','%in%','%like%','%between%','not %between%'), Multiple = TRUE, MaxVars = 1, PlotDropDown = PlotDropDown, Key = key)
            DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',shiny::isolate(PlotNums),'_3'), SelectedDefault = DataMuse:::FL_Default(tryCatch({shiny::isolate(data())}, error = function(x) NULL), x=tryCatch({FilterVariable_1_3()}, error = function(x) NULL)), Label = 'Logical Operation', Choices = c('<','>','<=','>=','%in%','%like%','%between%','not %between%'), Multiple = TRUE, MaxVars = 1, PlotDropDown = PlotDropDown, Key = key)
            DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',shiny::isolate(PlotNums),'_4'), SelectedDefault = DataMuse:::FL_Default(tryCatch({shiny::isolate(data())}, error = function(x) NULL), x=tryCatch({FilterVariable_1_4()}, error = function(x) NULL)), Label = 'Logical Operation', Choices = c('<','>','<=','>=','%in%','%like%','%between%','not %between%'), Multiple = TRUE, MaxVars = 1, PlotDropDown = PlotDropDown, Key = key)
            ...
            shiny::observeEvent(SelectedGroups1(), {
              ...
              if(length(shiny::isolate(SelectedGroups1())) == 0L) {
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = NULL, InputID=paste0('Levels_',shiny::isolate(PlotNums),'_1'), Choices=NULL, SelectedDefault=NULL)
              } else {
                if(Debug) {
                  print("here 2.a")
                ...
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = shiny::isolate(SelectedGroups1())[1L], InputID=paste0('Levels_',shiny::isolate(PlotNums),'_1'), Choices=sgs, SelectedDefault=NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('Levels_',shiny::isolate(PlotNums),'_1')]]) > 0L) input[[paste0('Levels_',shiny::isolate(PlotNums),'_1')]] else NULL)
              }
              ...
              if(length(shiny::isolate(SelectedGroups1())) %in% c(0L,1L)) {
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = NULL, InputID = paste0('Levels_',shiny::isolate(PlotNums),'_2'), Choices = NULL, SelectedDefault = NULL)
              } else {
                ...
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = shiny::isolate(SelectedGroups1())[2L], InputID=paste0('Levels_',shiny::isolate(PlotNums),'_2'), Choices=sgs, SelectedDefault=NULL, PlotDropDown = PlotDropDown, Key = key)#if(length(input[[paste0('Levels_',shiny::isolate(PlotNums),'_2')]]) > 0L) input[[paste0('Levels_',shiny::isolate(PlotNums),'_2')]] else NULL)
              }

              if(length(shiny::isolate(SelectedGroups1())) %in% c(0L:2L)) {
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = NULL, InputID = paste0('Levels_',shiny::isolate(PlotNums),'_3'), Choices = NULL, SelectedDefault = NULL)
              } else {
                ...
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = shiny::isolate(SelectedGroups1())[3L], InputID=paste0('Levels_',shiny::isolate(PlotNums),'_3'), Choices=sgs, SelectedDefault=NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('Levels_',shiny::isolate(PlotNums),'_3')]]) > 0L) input[[paste0('Levels_',shiny::isolate(PlotNums),'_3')]] else NULL)
              }
            }, ignoreNULL = FALSE)
            ...
            shiny::observeEvent(FilterVariable_1_1(), {
              if(length(FilterVariable_1_1()) <= 1L) {
                ...
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_1()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_1()))
                print("filter var 1")
                print(choices)
                if(length(choices) > 0L) {
                  print("here 1")
                  print(PlotDropDown[[key]][[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1')]])
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, InputID = paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1'), Label = 'Value 1', Choices = DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1')]] else NULL)
                } else {
                  print("here 2")
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, InputID = paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1'), Label = 'Value 1', Choices = NULL, SelectedDefault = NULL)#if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_1')]] else NULL)
                }
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_1()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_1()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple=Mult, InputID = paste0('FilterValue_',shiny::isolate(PlotNums),'_1_2'), Label='Value 2', Choices=DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_2')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple=Mult, InputID = paste0('FilterValue_',shiny::isolate(PlotNums),'_1_2'), Label='Value 2', Choices = NULL, SelectedDefault = NULL)#if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_1_2')]] else NULL)
                }
              }
            }, ignoreNULL = FALSE)
            ...
            shiny::observeEvent(FilterVariable_1_2(), {
              if(length(FilterVariable_1_2()) <= 1L) {
                ...
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_2()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_2()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple=Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_2_1'), Label='Value 1', Choices=DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_1')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple=Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_2_1'), Label='Value 1', Choices=NULL, SelectedDefault = NULL) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_1')]] else NULL)
                }
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_2()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({dshiny::isolate(ata())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_2()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple=Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_2_2'), Label='Value 2', Choices=DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_2')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple=Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_2_2'), Label='Value 2', Choices=NULL, SelectedDefault=NULL) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_2_2')]] else NULL)
                }
              }
            }, ignoreNULL = FALSE)
            ...
            shiny::observeEvent(FilterVariable_1_3(), {
              if(length(FilterVariable_1_3()) <= 1L) {
                ...
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_3()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_3()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_3_1'), Label='Value 1', Choices=DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_1')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_3_1'), Label='Value 1', Choices = NULL, SelectedDefault = NULL) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_1')]] else NULL)
                }
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_3()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_3()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input,Multiple = Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_3_2'), Label='Value 2', Choices=DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_2')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input,Multiple = Mult, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_3_2'), Label='Value 2', Choices = NULL, SelectedDefault = NULL) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_3_2')]] else NULL)
                }
              }
            }, ignoreNULL = FALSE)
            ...
            shiny::observeEvent(FilterVariable_1_4(), {
              if(length(FilterVariable_1_4())) {
                ...
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_4()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_4()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, MaxVars = 100, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_4_1'), Label='Value 1', Choices=DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)# if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_1')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, MaxVars = 100, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_4_1'), Label='Value 1', Choices = NULL, SelectedDefault = NULL) #if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_1')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_1')]] else NULL)
                }
                choices <- tryCatch({DataMuse:::KeyVarsInit(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_4()))$ChoiceInput}, error = function(x) NULL)
                Mult <- DataMuse:::GetFilterValueMultiple(tryCatch({shiny::isolate(data())}, error = function(x) NULL), VarName = shiny::isolate(FilterVariable_1_4()))
                if(length(choices) > 0L) {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, MaxVars = 100, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_4_2'), Label='Value 2', Choices = DataMuse::CharNull(choices), SelectedDefault = NULL, PlotDropDown = PlotDropDown, Key = key)#if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_2')]] else NULL)
                } else {
                  DataMuse:::PickerInput(session = session, Update = TRUE, input=input, Multiple = Mult, MaxVars = 100, InputID=paste0('FilterValue_',shiny::isolate(PlotNums),'_4_2'), Label='Value 2', Choices = NULL, SelectedDefault = NULL)#if(length(input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_2')]]) > 0L) input[[paste0('FilterValue_',shiny::isolate(PlotNums),'_4_2')]] else NULL)
                }
              }
            }, ignoreNULL = FALSE)
          })
  ...
  shiny::observeEvent(input$DataWrangling_SampleData, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.DW.SampleData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
  ...
  shiny::observeEvent(input$DataWrangling_SubsetData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.SubsetData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_AggregateData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.AggregateData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_JoinData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.JoinData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_UnionData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.UnionData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_MeltData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.MeltData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_CastData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.CastData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_RemoveData, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.RemoveData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_TimeSeriesFill, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.TimeSeriesFill(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWrangling_TimeSeriesRoll, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.TimeSeriesRoll(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_DeleteColumns, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.DW.DeleteColumns(input,output,session,DataList,DataWranglingCode,TabCount = NumTabs, CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_ConcatColumns, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.DW.ConcatenateColumns(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_MetaProgramming, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.DW.MetaProgramming(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_SortData, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.DW.SortData(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_RenameColumns, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.DW.RenameColumns(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_TimeTrendColumn, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.DW.TimeTrendColumn(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$DataWranglingButton_TypeCasting, {
    if(!exists('DataWranglingCode')) DataWranglingCode <- list()
    if(Debug) print('DataWranglingButton_TimeTrendColumn')
    Output <- tryCatch({DataMuse:::Shiny.DW.TypeCast(input,output,session,DataList,DataWranglingCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      DataList <- Output$DataList; DataList <<- DataList; DataWranglingCode <- Output$CodeList; DataWranglingCode <<- DataWranglingCode
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_CalendarVariables, {
    ...
    if(Debug) {
      ...
      Output <- DataMuse:::Shiny.FE.Date.Calendar(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)
    } else {
      Output <- tryCatch({DataMuse:::Shiny.FE.Date.Calendar(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    }
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_HolidayVariables, {
    ...
    Output <- tryCatch({DataMuse:::Shiny.FE.Date.Holiday(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_PercRank, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Numeric.PercentRank(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_Standardize, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Numeric.Standardize(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_Interaction, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Numeric.Interactions(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_Transformations, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Numeric.Transformations(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_PartialDummies, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Categorical.Dummify(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_CategoricalEncoding, {
    Output <- tryCatch({DataMuse:::Shiny.FE.CrossRow.CategoricalEncoding(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_AutoLagRollMode, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.CrossRow.RollingMode(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_AutoLagRollStats, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.CrossRow.RollingStats(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_AutoDiff, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.CrossRow.Differencing(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_ModelDataPrep, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.ModelDataPrep(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_AutoDataPartition, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.PartitionData(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_AutoEncoder_H2O, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.DimReduction.AutoEncoder.H2O(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_IsolationForest_H2O, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.AnomalyDetection.IsolationForest.H2O(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_Kmeans_H2O, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Clustering.Kmeans.H2O(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$FeatureEngineeringButton_Word2Vec_H2O, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    Output <- tryCatch({DataMuse:::Shiny.FE.Word2Vec.H2O(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
    } else {
  ...
  shiny::observeEvent(input$NLPButton_TextSummary, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    if(Debug) {
      Output <- DataMuse:::Shiny.NLP.TextSummary(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)
    } else {
      Output <- tryCatch({DataMuse:::Shiny.NLP.TextSummary(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    }
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
  ...
  shiny::observeEvent(input$NLPButton_Sentiment, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    if(Debug) {
      Output <- DataMuse:::Shiny.NLP.Sentiment(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)
    } else {
      Output <- tryCatch({DataMuse:::Shiny.NLP.Sentiment(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    }
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
  ...
  shiny::observeEvent(input$NLPButton_Readability, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    if(Debug) {
      Output <- DataMuse:::Shiny.NLP.Readability(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)
    } else {
      Output <- tryCatch({DataMuse:::Shiny.NLP.Readability(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    }
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
  ...
  shiny::observeEvent(input$NLPButton_LexicalDiversity, {
    if(!exists('FeatureEngineeringCode')) FeatureEngineeringCode <- list()
    if(Debug) {
      Output <- DataMuse:::Shiny.NLP.LexicalDiversity(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)
    } else {
      Output <- tryCatch({DataMuse:::Shiny.NLP.LexicalDiversity(input,output,session,DataList,FeatureEngineeringCode,CacheDir=CacheDir,CacheName=CacheName,Debug=Debug)}, error = function(x) NULL)
    }
    if(length(Output) > 0L) {
      FeatureEngineeringCode <- Output$CodeList; FeatureEngineeringCode <<- FeatureEngineeringCode; DataList <- Output$DataList; DataList <<- DataList
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Success", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
  ...
  shiny::observeEvent(input$BuildModels_CatBoost, {
    ...
    shiny::withProgress(message = 'CatBoost ML has begun..', value = 0, {
      ArgsList <- list()
      CatBoost_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['CatBoost_TargetType']]}, error=function(x) NULL), Type='character', Default='MultiClass')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['CatBoost_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(CatBoost_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, CatBoost_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'CatBoost', wd = WorkingDirectory)
          } else if(CatBoost_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, CatBoost_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'CatBoost', wd = WorkingDirectory)
          } else if(CatBoost_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, CatBoost_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'CatBoost', wd = WorkingDirectory)
          }
        } else {
          if(CatBoost_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, CatBoost_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'CatBoost', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(CatBoost_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, CatBoost_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'CatBoost', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(CatBoost_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, CatBoost_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'CatBoost', wd = WorkingDirectory)}, error = function(x) NULL)
          }
        }
        ...
        if(length(Output) > 0L) {
          ...
          DataMuse::SelectizeInput(session, input, Update = TRUE, InputID = 'ScoreML_Model', Label = 'Select Model', Choices = names(ModelOutputList), SelectedDefault = NULL)
          DataMuse::SelectizeInput(session, input, Update = TRUE, InputID = 'ScoreML_Data', Label = 'Select Data', Choices = names(DataList), SelectedDefault = NULL)
  ...
  shiny::observeEvent(input$BuildModels_XGBoost,  {
    ...
    shiny::withProgress(message = 'XGBoost ML has begun..', value = 0, {
      ArgsList <- list()
      XGBoost_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['XGBoost_TargetType']]}, error=function(x) NULL), Type='character', Default='MultiClass')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['XGBoost_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(XGBoost_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, XGBoost_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'XGBoost')
          } else if(XGBoost_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, XGBoost_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'XGBoost')
          } else if(XGBoost_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, XGBoost_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'XGBoost')
          }
        } else {
          if(XGBoost_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, XGBoost_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'XGBoost')}, error = function(x) NULL)
          } else if(XGBoost_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, XGBoost_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'XGBoost')}, error = function(x) NULL)
          } else if(XGBoost_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, XGBoost_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'XGBoost')}, error = function(x) NULL)
          }
        }
  ...
  shiny::observeEvent(input$BuildModels_LightGBM, {
    ...
    shiny::withProgress(message = 'LightGBM ML has begun..', value = 0, {
      ArgsList <- list()
      LightGBM_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['LightGBM_TargetType']]}, error=function(x) NULL), Type='character', Default='MultiClass')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['LightGBM_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(LightGBM_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, LightGBM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'LightGBM')
          } else if(LightGBM_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, LightGBM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'LightGBM')
          } else if(LightGBM_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, LightGBM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'LightGBM')
          }
        } else {
          if(LightGBM_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, LightGBM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'LightGBM')}, error = function(x) NULL)
          } else if(LightGBM_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, LightGBM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'LightGBM')}, error = function(x) NULL)
          } else if(LightGBM_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, LightGBM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'LightGBM')}, error = function(x) NULL)
          }
        }
  ...
  shiny::observeEvent(input$BuildModels_H2O_DRF, {
    ...
    shiny::withProgress(message = 'H2O_DRF ML has begun..', value = 0, {
      ArgsList <- list()
      H2O_DRF_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_DRF_TargetType']]}, error=function(x) NULL), Type='character', Default='MultiClass')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_DRF_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(H2O_DRF_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_DRF_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_DRF', wd = WorkingDirectory)
          } else if(H2O_DRF_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_DRF_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_DRF', wd = WorkingDirectory)
          } else if(H2O_DRF_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_DRF_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_DRF', wd = WorkingDirectory)
          }
        } else {
          if(H2O_DRF_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_DRF_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_DRF', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(H2O_DRF_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_DRF_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_DRF', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(H2O_DRF_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_DRF_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_DRF', wd = WorkingDirectory)}, error = function(x) NULL)
          }
        }
  ...
  shiny::observeEvent(input$BuildModels_H2O_GBM, {
    ...
    shiny::withProgress(message = 'H2O_GBM ML has begun..', value = 0, {
      ArgsList <- list()
      H2O_GBM_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_GBM_TargetType']]}, error=function(x) NULL), Type='character', Default='MultiClass')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_GBM_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(H2O_GBM_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GBM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_GBM', wd = WorkingDirectory)
          } else if(H2O_GBM_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GBM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_GBM', wd = WorkingDirectory)
          } else if(H2O_GBM_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GBM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_GBM', wd = WorkingDirectory)
          }
        } else {
          if(H2O_GBM_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GBM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_GBM', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(H2O_GBM_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GBM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_GBM', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(H2O_GBM_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GBM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_GBM', wd = WorkingDirectory)}, error = function(x) NULL)
          }
        }
  ...
  shiny::observeEvent(input$BuildModels_H2O_GLM, {
    ...
    shiny::withProgress(message = 'H2O_GLM ML has begun..', value = 0, {
      ArgsList <- list()
      H2O_GLM_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_GLM_TargetType']]}, error=function(x) NULL), Type='character', Default='Regression')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_GLM_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(H2O_GLM_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GLM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_GLM', wd = WorkingDirectory)
          } else if(H2O_GLM_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GLM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_GLM', wd = WorkingDirectory)
          } else if(H2O_GLM_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GLM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_GLM', wd = WorkingDirectory)
          }
        } else {
          if(H2O_GLM_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GLM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_GLM', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(H2O_GLM_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GLM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_GLM', wd = WorkingDirectory)}, error = function(x) NULL)
          } else if(H2O_GLM_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_GLM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_GLM', wd = WorkingDirectory)}, error = function(x) NULL)
          }
        }
  ...
  shiny::observeEvent(input$BuildModels_H2O_HGLM, {
    ...
    shiny::withProgress(message = 'H2O_HGLM ML has begun..', value = 0, {
      ArgsList <- list()
      H2O_HGLM_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_HGLM_TargetType']]}, error=function(x) NULL), Type='character', Default='Regression')
      n <- DataMuse:::ReturnParam(xx=tryCatch({input[['H2O_HGLM_Runs']]}, error=function(x) NULL), Type='numeric', Default=1L)
      for(run in seq_len(n)) {

        # Build Model
        if(Debug) {
          if(H2O_HGLM_TargetType == "Regression") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_HGLM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_HGLM')
          } else if(H2O_HGLM_TargetType == "Binary Classification") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_HGLM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_HGLM')
          } else if(H2O_HGLM_TargetType == "MultiClass") {
            Output <- DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_HGLM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_HGLM')
          }
        } else {
          if(H2O_HGLM_TargetType == "Regression") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_HGLM_TargetType, ML_RegressionTable, run, n, Debug, Algo = 'H2O_HGLM')}, error = function(x) NULL)
          } else if(H2O_HGLM_TargetType == "Binary Classification") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_HGLM_TargetType, ML_ClassificationTable, run, n, Debug, Algo = 'H2O_HGLM')}, error = function(x) NULL)
          } else if(H2O_HGLM_TargetType == "MultiClass") {
            Output <- tryCatch({DataMuse:::Shiny.ML.Trainer(input, output, DataList, ArgsList, MachineLearningCode, ModelOutputList, H2O_HGLM_TargetType, ML_MultiClassTable, run, n, Debug, Algo = 'H2O_HGLM')}, error = function(x) NULL)
          }
        }
  ...
  shiny::observeEvent(input$BuildModels_CausalMediation, {
    ...
    shiny::withProgress(message = 'Causal Mediation has begun..', value = 0, {
      ...
      # ArgsList <- list()
      x <- DataMuse:::ReturnParam(xx = tryCatch({input[['CausalMediation_data']]}, error=function(x) NULL), Type='character', Default=NULL)
      if(length(x) > 0L) {

        # Values
        data <- DataList[[x]]
        OutcomeTargetVariable <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_OutcomeTargetVariable']]}, error=function(x) NULL), Type='character', Default=NULL)
        TreatmentVariable <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_TreatmentVariable']]}, error=function(x) NULL), Type='character', Default=NULL)
        MediatorVariable <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_MediatorVariable']]}, error=function(x) NULL), Type='character', Default=NULL)
        Covariates <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_Covariates']]}, error=function(x) NULL), Type='character', Default=NULL)
        MM_TreatmentCovariates <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_MM_TreatmentCovariates']]}, error=function(x) NULL), Type='character', Default=NULL)
        OM_TreatmentCovariates <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_OM_TreatmentCovariates']]}, error=function(x) NULL), Type='character', Default=NULL)
        OM_MediatorCovariates <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_OM_MediatorCovariates']]}, error=function(x) NULL), Type='character', Default=NULL)
        SurvivalEventVariable <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_SurvivalEventVariable']]}, error=function(x) NULL), Type='character', Default=NULL)
        UnTreated_ReferenceIndicator <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_UnTreated_ReferenceIndicator']]}, error=function(x) NULL), Type='numeric', Default=0)
        Treated_ReferenceIndicator <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_Treated_ReferenceIndicator']]}, error=function(x) NULL), Type='numeric', Default=1)
        Mediator_ControlDirectEffectLevel <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_Mediator_ControlDirectEffectLevel']]}, error=function(x) NULL), Type='numeric', Default=0)
        Covariate_NaturalDirectIndirect <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_Covariate_NaturalDirectIndirect']]}, error=function(x) NULL), Type='numeric', Default=NULL)
        MediatorTargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_MediatorTargetType']]}, error=function(x) NULL), Type='character', Default='linear')
        OutcomeTargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_OutcomeTargetType']]}, error=function(x) NULL), Type='character', Default='linear')
        TreatmentMediatorInteraction <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_TreatmentMediatorInteraction']]}, error=function(x) NULL), Type='logical', Default=TRUE)
        CaseControlSourceData <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_CaseControlSourceData']]}, error=function(x) NULL), Type='logical', Default=FALSE)
        RemoveNA <- DataMuse:::ReturnParam(xx=tryCatch({input[['CausalMediation_RemoveNA']]}, error=function(x) NULL), Type='logical', Default=FALSE)
        ...
        if(length(Output) > 0L) {
          ...
          MachineLearningCode <- tryCatch({DataMuse:::Shiny.CodePrint.Collect(y = MachineLearningCode, x = paste0(
            ...
            "data = DataList[['data']],\n  ",
            "OutcomeTargetVariable = ", DataMuse:::CEP(OutcomeTargetVariable),",\n  ",
            "TreatmentVariable = ", DataMuse:::CEP(TreatmentVariable), ",\n  ",
            "MediatorVariable = ", DataMuse:::CEP(MediatorVariable), ",\n  ",
            "Covariates = ", DataMuse:::ExpandText(Covariates), ",\n  ",
            "MM_TreatmentCovariates = ", DataMuse:::ExpandText(MM_TreatmentCovariates), ",\n  ",
            "OM_TreatmentCovariates = ", DataMuse:::ExpandText(OM_TreatmentCovariates), ",\n  ",
            "OM_MediatorCovariates = ", DataMuse:::ExpandText(OM_MediatorCovariates), ",\n  ",
            "SurvivalEventVariable = ", DataMuse:::CEPP(SurvivalEventVariable), ",\n  ",
            "UnTreated_ReferenceIndicator = ", DataMuse:::CEPP(UnTreated_ReferenceIndicator), ",\n  ",
            "Treated_ReferenceIndicator = ", DataMuse:::CEPP(Treated_ReferenceIndicator), ",\n  ",
            "Mediator_ControlDirectEffectLevel = ", DataMuse:::CEPP(Mediator_ControlDirectEffectLevel), ",\n  ",
            "Covariate_NaturalDirectIndirect = ", DataMuse:::CEPP(Covariate_NaturalDirectIndirect), ",\n  ",
            "MediatorTargetType = ", DataMuse:::CEP(MediatorTargetType), ",\n  ",
            "OutcomeTargetType = ", DataMuse:::CEP(OutcomeTargetType), ",\n  ",
            "TreatmentMediatorInteraction = ", DataMuse:::CEPP(TreatmentMediatorInteraction), ",\n  ",
            "CaseControlSourceData = ", DataMuse:::CEPP(CaseControlSourceData), ",\n  ",
            "RemoveNA = ", DataMuse:::CEPP(RemoveNA), ")\n\n"))}, error = function(x) MachineLearningCode)
  ...
  shiny::observeEvent(input$BuildModels_MixedEffects, {
    ...
    shiny::withProgress(message = 'Mixed Effects has begun..', value = 0, {
      ...
      # ArgsList <- list()
      x <- DataMuse:::ReturnParam(xx = tryCatch({input[['MixedEffects_data']]}, error=function(x) NULL), Type='character', Default=NULL)
      if(length(x) > 0L) {

        # Values
        data <- DataList[[x]][['data']]
        x <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_ValidationData']]}, error=function(x) NULL), Type='character', Default=NULL)
        if(length(x) > 0L) ValidationData <- DataList[[x]][['data']] else ValidationData <- NULL
        x <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_TestData']]}, error=function(x) NULL), Type='character', Default=NULL)
        if(length(x) > 0L) TestData <- DataList[[x]][['data']] else TestData <- NULL

        MixedEffects_TargetColumnName <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_TargetColumnName']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_TargetType <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_TargetType']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_ModelID <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_ModelID']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_FixedEffects <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_FixedEffects']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_RandomEffects <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_RandomEffects']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_PrimaryDateColumn <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_PrimaryDateColumn']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_WeightsColumn <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_WeightsColumn']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_InterceptInclude <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_InterceptInclude']]}, error=function(x) NULL), Type='logical', Default=TRUE)

        MixedEffects_Nest <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_Nest']]}, error=function(x) NULL), Type='character', Default=NULL)
        MixedEffects_CollapseEPV <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_CollapseEPV']]}, error=function(x) NULL), Type='logical', Default=TRUE)
        MixedEffects_KeepSubStats <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_Lmer']]}, error=function(x) NULL), Type='logical', Default=TRUE)
        MixedEffects_Lmer <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_Lmer']]}, error=function(x) NULL), Type='logical', Default=TRUE)
        MixedEffects_LmerType <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_LmerType']]}, error=function(x) NULL), Type='character', Default='add')
        MixedEffects_FastMixedEffects <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_FastMixedEffects']]}, error=function(x) NULL), Type='logical', Default=TRUE)
        MixedEffects_PolyN <- DataMuse:::ReturnParam(xx=tryCatch({input[['MixedEffects_PolyN']]}, error=function(x) NULL), Type='numeric', Default=NULL)
        ...
        if(length(Output) > 0L) {
          ...
          MachineLearningCode <- DataMuse:::Shiny.CodePrint.Collect(y = MachineLearningCode, x = paste0(
            ...
            "data = DataList[['data']],\n  ",
            "TargetType = ", DataMuse:::CEP(MixedEffects_TargetType),",\n  ",
            "TargetVariable = ", DataMuse:::CEP(MixedEffects_TargetColumnName),",\n  ",
            "RandomEffects = ", DataMuse:::ExpandText(MixedEffects_RandomEffects), ",\n  ",
            "FixedEffects = ", DataMuse:::ExpandText(MixedEffects_FixedEffects), ",\n  ",
            "Nest = ", DataMuse:::ExpandText(MixedEffects_Nest), ",\n  ",
            "CollapseEPV = ", DataMuse:::CEPP(MixedEffects_CollapseEPV), ",\n  ",
            "KeepSubStats = ", DataMuse:::CEPP(MixedEffects_KeepSubStats), ",\n  ",
            "Lmer = ", DataMuse:::CEPP(MixedEffects_Lmer), ",\n  ",
            "LmerType = ", DataMuse:::CEP(MixedEffects_LmerType), ",\n  ",
            "Fme = ", DataMuse:::CEPP(MixedEffects_FastMixedEffects), ",\n  ",
            "PolyN = ", DataMuse:::CEPP(MixedEffects_PolyN), ",\n  ",
            "Debug = FALSE)\n\n"))
  ...
  shiny::observeEvent(input$ScoreML_Run, {
    ...
    if(length(DataList) > 0L) {
      if(Debug) {
        print("ML Scoring 2")
        Output <- DataMuse::Shiny.ML.Scoring(
          input,
          output,
          DataList = DataList,
        ...
        Output <- tryCatch({DataMuse::Shiny.ML.Scoring(
          input,
          output,
          DataList = DataList,
          CodeList = MachineLearningCode,
  ...
  shiny::observeEvent(input$BuildModels_TBATS, {
    ...
    shiny::withProgress(message = 'TBATS FC has begun..', value = 0, {
      ...
      alname <- DataMuse::ReturnParam(xx = input[['TBATS_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['TBATS_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Forecast")) {
        ...
        if(!Skipp) {
          ...
          # Run FC System
          if(DebugFC) {
            Output <- DataMuse::Shiny.FC.SS(input, output, DataList, TBATSFC, ForecastingCode, DebugFC, Algo = 'TBATS', wd = WorkingDirectory)
          } else {
            Output <- tryCatch({DataMuse::Shiny.FC.SS(input, output, DataList, TBATSFC, ForecastingCode, DebugFC, Algo = 'TBATS', wd = WorkingDirectory)}, error = function(x) NULL)
          }
          ...
          if(length(Output) > 0L) {
            ...
            if(length(TBATSFC) > 0L) {
              ...
              for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection",i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
              DataMuse:::SelectizeInput(session = session, input = input, Update = TRUE, InputID = "TBATS_ArgsList", Label = "Tuning Grid", Choices = names(ModelOutputList), SelectedDefault = NULL, Multiple = TRUE, MaxVars = 100L)
  ...
  shiny::observeEvent(input$BuildModels_SARIMA, {
    ...
    shiny::withProgress(message = 'SARIMA FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['SARIMA_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['SARIMA_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Forecast")) {
        shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Please supply a value to ArgsList for this RunMode", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
      } else {
        ...
        if(!Skipp) {
          ...
          # Run FC System
          if(DebugFC) {
            Output <- DataMuse::Shiny.FC.SS(input, output, DataList, SARIMAFC, ForecastingCode, DebugFC, Algo = 'SARIMA', wd = WorkingDirectory)
          } else {
            Output <- tryCatch({DataMuse::Shiny.FC.SS(input, output, DataList, SARIMAFC, ForecastingCode, DebugFC, Algo = 'SARIMA', wd = WorkingDirectory)}, error = function(x) NULL)
          }
          ...
          if(length(Output) > 0L) {
            ...
            if(length(SARIMAFC) > 0L) {
              ...
              ModelOutputList <<- ModelOutputList; rm(Output); gc()
              for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection",i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
              DataMuse:::SelectizeInput(session = session, input = input, Update = TRUE, InputID = "SARIMA_ArgsList", Label = "Tuning Grid", Choices = names(ModelOutputList), SelectedDefault = NULL, Multiple = TRUE, MaxVars = 100L)
  ...
  shiny::observeEvent(input$BuildModels_ETS, {
    ...
    shiny::withProgress(message = 'ETS FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['ETS_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['ETS_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Forecast")) {
        shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Please supply a value to ArgsList for this RunMode", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
      } else {
        ...
        if(!Skipp) {
          ...
          # Run FC System
          if(DebugFC) {
            Output <- DataMuse::Shiny.FC.SS(input, output, DataList, ETSFC, ForecastingCode, DebugFC, Algo = 'ETS', wd = WorkingDirectory)
          } else {
            Output <- tryCatch({DataMuse::Shiny.FC.SS(input, output, DataList, ETSFC, ForecastingCode, DebugFC, Algo = 'ETS', wd = WorkingDirectory)}, error = function(x) NULL)
          }
          ...
          if(length(Output) > 0L) {
            ...
            if(length(ETSFC) > 0L) {
              ...
              ModelOutputList <<- ModelOutputList; rm(Output); gc()
              for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection",i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
              DataMuse:::SelectizeInput(session = session, input = input, Update = TRUE, InputID = "ETS_ArgsList", Label = "Tuning Grid", Choices = names(ModelOutputList), SelectedDefault = NULL, Multiple = TRUE, MaxVars = 100L)
  ...
  shiny::observeEvent(input$BuildModels_ARFIMA, {
    ...
    shiny::withProgress(message = 'ARFIMA FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['ARFIMA_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['ARFIMA_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Forecast")) {
        shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Please supply a value to ArgsList for this RunMode", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
      } else {
        ...
        #     ModelOutputList: main list for storing models in DataMuse. Can contain ML models, Panel Models, and Single Series Models
        ...
        if(!Skipp) {
          ...
          # Run FC System
          if(DebugFC) {
            Output <- DataMuse::Shiny.FC.SS(input, output, DataList, ARFIMAFC, ForecastingCode, DebugFC, Algo = 'ARFIMA', wd = WorkingDirectory)
          } else {
            Output <- tryCatch({DataMuse::Shiny.FC.SS(input, output, DataList, ARFIMAFC, ForecastingCode, DebugFC, Algo = 'ARFIMA', wd = WorkingDirectory)}, error = function(x) NULL)
          }
          ...
          if(length(Output) > 0L) {
            ...
            if(length(ARFIMAFC) > 0L) {
              ...
              ModelOutputList <<- ModelOutputList; rm(Output); gc()
              for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection",i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
              DataMuse:::SelectizeInput(session = session, input = input, Update = TRUE, InputID = "ARFIMA_ArgsList", Label = "Tuning Grid", Choices = names(ModelOutputList), SelectedDefault = NULL, Multiple = TRUE, MaxVars = 100L)
  ...
  shiny::observeEvent(input$BuildModels_NNET, {
    ...
    shiny::withProgress(message = 'NNET FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['NNET_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['NNET_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Forecast")) {
        shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Please supply a value to ArgsList for this RunMode", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
      } else {
        ...
        if(!Skipp) {
          ...
          # Run FC System
          if(DebugFC) {
            Output <- DataMuse::Shiny.FC.SS(input, output, DataList, NNETFC, ForecastingCode, DebugFC, Algo = 'NNET', wd = WorkingDirectory)
          } else {
            Output <- tryCatch({DataMuse::Shiny.FC.SS(input, output, DataList, NNETFC, ForecastingCode, DebugFC, Algo = 'NNET', wd = WorkingDirectory)}, error = function(x) NULL)
          }
          ...
          if(length(Output) > 0L) {
            ...
            if(length(NNETFC) > 0L) {
              ...
              print(names(ModelOutputList))
              for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection",i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
              DataMuse:::SelectizeInput(session = session, input = input, Update = TRUE, InputID = "NNET_ArgsList", Label = "Tuning Grid", Choices = names(ModelOutputList), SelectedDefault = NULL, Multiple = TRUE, MaxVars = 100L)
  ...
  shiny::observeEvent(input$BuildModels_CatBoostCARMA, {
    ...
    shiny::withProgress(message = 'CatBoost FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['CatBoostCARMA_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['CatBoostCARMA_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Retrain","Forecast","Forecast+Retrain")) {
        ...
        # Run FC System
        if(DebugFC) {
          Output <- DataMuse::Shiny.FC.CARMA(input, output, DataList, CatBoostFC, ForecastingCode, DebugFC, Algo = 'CatBoost', wd = WorkingDirectory)
        } else {
          Output <- tryCatch({DataMuse::Shiny.FC.CARMA(input, output, DataList, CatBoostFC, ForecastingCode, DebugFC, Algo = 'CatBoost', wd = WorkingDirectory)}, error = function(x) NULL)
        }
  ...
  shiny::observeEvent(input$BuildModels_XGBoostCARMA, {
    ...
    shiny::withProgress(message = 'XGBoost FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['XGBoostCARMA_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['XGBoostCARMA_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Retrain","Forecast","Forecast+Retrain")) {
        shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Please supply a value to ArgsList for this RunMode", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
      } else {
        ...
        # Run FC System
        if(DebugFC) {
          Output <- DataMuse::Shiny.FC.CARMA(input, output, DataList, XGBoostFC, ForecastingCode, DebugFC, Algo = 'XGBoost', wd = WorkingDirectory)
        } else {
          Output <- tryCatch({DataMuse::Shiny.FC.CARMA(input, output, DataList, XGBoostFC, ForecastingCode, DebugFC, Algo = 'XGBoost', wd = WorkingDirectory)}, error = function(x) NULL)
        }
  ...
  shiny::observeEvent(input$BuildModels_LightGBMCARMA, {
    ...
    shiny::withProgress(message = 'LightGBM FC has begun..', value = 0, {
      ...
      # If a user can request one it must be available. Error handling just in case
      alname <- DataMuse::ReturnParam(xx = input[['LightGBMCARMA_ArgsList']], Type = 'character', Debug = DebugFC)
      runm   <- DataMuse::ReturnParam(xx = input[['LightGBMCARMA_RunMode']], Type = 'character', Debug = DebugFC)
      if(length(alname) == 0L && runm %in% c("Retrain","Forecast","Forecast+Retrain")) {
        shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = "Please supply a value to ArgsList for this RunMode", btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "40%")
      } else {
        ...
        # Run FC System
        if(DebugFC) {
          Output <- DataMuse::Shiny.FC.CARMA(input, output, DataList, LightGBMFC, ForecastingCode, DebugFC, Algo = 'LightGBM', wd = WorkingDirectory)
        } else {
          Output <- tryCatch({DataMuse::Shiny.FC.CARMA(input, output, DataList, LightGBMFC, ForecastingCode, DebugFC, Algo = 'LightGBM', wd = WorkingDirectory)}, error = function(x) NULL)
        }
  ...
  shiny::observeEvent(input$SaveSessionButton, {
    ...
    SaveSessionName <- DataMuse:::ReturnParam(xx = tryCatch({input$SaveSessionName}, error = function(x) NULL), Type = 'character', Debug = Debug)
    if(length(SaveSessionName) > 0L) {
      ...
      dataCheck <- DataMuse:::ReturnParam(xx = tryCatch({input$LoadSession_IncludeData}, error = function(x) TRUE), Type = 'logical', Debug = Debug)
      ...
      for(i in seq_len(NumPlotsAvailable)) {
        PlotType <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('Plot', i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
        if(length(PlotType) > 0L) {
          key <- paste0(PlotType, i)
          PlotDropDown[[key]][[paste0('Symbols', i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('Symbols', i)]]}, error=function(x) NULL), Type='character', Default=NULL)
          PlotDropDown[[key]][[paste0('StockMetric', i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('StockMetric', i)]]}, error=function(x) NULL), Type='character', Default=NULL)
          PlotDropDown[[key]][[paste0('StockTimeAgg', i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('StockTimeAgg', i)]]}, error=function(x) NULL), Type='character', Default='days')
          PlotDropDown[[key]][[paste0('StockDateRange', i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('StockDateRange', i)]]}, error=function(x) NULL), Type='date', Default=c(as.character(Sys.Date()-365), as.character(Sys.Date())))
          PlotDropDown[[key]][[paste0("Plot",i,"_SelectData")]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("Plot",i,"_SelectData")]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('Plot',i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('Plot',i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('YVar',i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('YVar',i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('DualYVar',i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('DualYVar',i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('XVar',i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('XVar',i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('ZVar',i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('ZVar',i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('GroupVars',i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('GroupVars',i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("YVarTrans",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("YVarTrans",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("DualYVarTrans",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("DualYVarTrans",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("XVarTrans",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("XVarTrans",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("ZVarTrans",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("ZVarTrans",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("AggMethod",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("AggMethod",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("FacetRows",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("FacetRows",i)]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("FacetCols",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("FacetCols",i)]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('Levels_',i,'_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('Levels_',i,'_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('Levels_',i,'_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('Levels_',i,'_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('Levels_',i,'_3')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('Levels_',i,'_3')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)

          PlotDropDown[[key]][[paste0("Title",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("Title",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("ShowLabels",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("ShowLabels",i)]]}, error = function(x) NULL), Type='logical', Default=FALSE, Debug = TRUE)
          PlotDropDown[[key]][[paste0("YAxisTitle",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("YAxisTitle",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0("XAxisTitle",i)]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0("XAxisTitle",i)]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)

          # Filter vars
          PlotDropDown[[key]][[paste0('FilterVariable_',i,'_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterVariable_',i,'_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('FilterLogic_',i,'_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterLogic_',i,'_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          ...
          if(FilterVariable1Class %in% c("numeric","integer")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_1')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_2')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("character","factor")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("Date","IDate")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_1')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_2')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("IDateTime","POSIXct","POSIXt")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_1')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_2')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("logical")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_1')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_1_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_1_2')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
          }

          PlotDropDown[[key]][[paste0('FilterVariable_',i,'_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterVariable_',i,'_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('FilterLogic_',i,'_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterLogic_',i,'_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          ...
          if(FilterVariable1Class %in% c("numeric","integer")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_1')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_2')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("character","factor")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("Date","IDate")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_1')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_2')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("IDateTime","POSIXct","POSIXt")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_1')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_2')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("logical")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_1')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_2_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_2_2')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
          }

          PlotDropDown[[key]][[paste0('FilterVariable_',i,'_3')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterVariable_',i,'_3')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('FilterLogic_',i,'_3')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterLogic_',i,'_3')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          ...
          if(FilterVariable1Class %in% c("numeric","integer")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_1')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_2')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("character","factor")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("Date","IDate")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_1')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_2')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("IDateTime","POSIXct","POSIXt")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_1')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_2')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("logical")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_1')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_3_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_3_2')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
          }

          PlotDropDown[[key]][[paste0('FilterVariable_',i,'_4')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterVariable_',i,'_4')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          PlotDropDown[[key]][[paste0('FilterLogic_',i,'_4')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterLogic_',i,'_4')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          ...
          if(FilterVariable1Class %in% c("numeric","integer")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_1')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_2')]]}, error = function(x) NULL), Type='numeric', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("character","factor")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_1')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_2')]]}, error = function(x) NULL), Type='character', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("Date","IDate")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_1')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_2')]]}, error = function(x) NULL), Type='date', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("IDateTime","POSIXct","POSIXt")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_1')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_2')]]}, error = function(x) NULL), Type='posix', Default=NULL, Debug = TRUE)
          } else if(FilterVariable1Class %in% c("logical")) {
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_1')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_1')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
            PlotDropDown[[key]][[paste0('FilterValue_',i,'_4_2')]] <- DataMuse:::ReturnParam(xx=tryCatch({input[[paste0('FilterValue_',i,'_4_2')]]}, error = function(x) NULL), Type='logical', Default=NULL, Debug = TRUE)
          }
        }
      }
      ...
      DataMuseSession <- list()
      DataMuseSession[['InputListNames']] <- c(
      ...
      for(SessionObjectName in DataMuseSession[['InputListNames']]) {
  ...
  shiny::observeEvent(input$LoadSessionButton, {
    ...
    shiny::withProgress(message = 'Loading session...', value = 0, {
      ...
      LoadSessionName <- basename(DataMuse:::ReturnParam(xx = tryCatch({input$LoadSessionName}, error = function(x) NULL), Type = 'character', Debug = DebugSessionLoad))
      SessionPath <- file.path(WorkingDirectory, LoadSessionName)
      data_check <- DataMuse:::ReturnParam(xx = tryCatch({input$LoadSession_IncludeData}, error = function(x) NULL), Type = 'logical', Debug = DebugSessionLoad, Default = TRUE)
      ...
      if(length(LoadSessionName) > 0L) {
        ...
        if(file_check) {
          ...
          for(SessionObjectName in DataMuseSession[['InputListNames']]) if(exists(SessionObjectName) && length(SessionObjectName) > 0L) eval(parse(text = paste0(SessionObjectName, " <<- ", SessionObjectName)))
            ...
            if(length(DataListNames) > 0L) {
              for(nam in DataListNames) {
                if(file.exists(file.path(SessionPath, paste0(gsub(pattern = '.csv', replacement = '', x = nam), '.csv')))) {
                  ...
                  DataList <- tryCatch({DataMuse:::DM.DataListUpdate(DataList, nam)}, error = function(x) DataList)
                  DataList <<- DataList
                }
              }
              for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection",i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
          ...
          # Table Tabs Update
          for(i in seq_len(NumTableTabsCurrent)) {
            DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('DataOutputSelection',i), Label = 'Display Data', Choices = NULL, SelectedDefault = TableTabList[[paste0("DataOutputSelection",i)]], Multiple = TRUE, MaxVars = 100L)
          ...
          for(i in seq_len(NumMLTabsCurrent)) {
            ...
            if(data.table::is.data.table(d)) {
              ColTypes <- unique(DataMuse:::ColTypes(d))
              for(j in seq_along(ColTypes)) ChoiceList[[ColTypes[j]]] <- DataMuse:::ColNameFilter(d, Types = ColTypes[j])
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('MLReportsModelSelection',i), Label = 'Model Selection', Choices = ChoiceList, SelectedDefault = MLModelSelection, Multiple = FALSE, MaxVars = 1L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0("MLOutputSelection",i), Label = "Evaluation Objects", Choices = c("Model Comparison Metrics", "Evaluation Plots", "Partial Dependence Plots"), SelectedDefault = MLTabList[[paste0("MLOutputSelection",i)]], Multiple = TRUE, MaxVars = 100L)
              DataMuse::NumericInput(session = session, input = input, Update = TRUE, InputID = paste0("SampleSize", i), Label = 'Sample Size', Value = MLTabList[[paste0("SampleSize",i)]], Min = 1, Max = 10000000, Step = 10000)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('PDPVariables',i), Label = 'PDP Variables', Choices = ChoiceList, SelectedDefault = MLTabList[[paste0("PDPVariables",i)]], Multiple = TRUE, MaxVars = 100L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0("TrainDataInclude", i), Label = 'Include Train Data?', Choices = c(TRUE,FALSE), SelectedDefault = MLTabList[[paste0("TrainDataInclude",i)]], Multiple = FALSE)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('GroupVariableInclude',i), Label = 'By-Variable Include?', Choices = ChoiceList, SelectedDefault = MLTabList[[paste0("GroupVariableInclude",i)]], Multiple = TRUE, MaxVars = 1L)
            }
          ...
          for(i in seq_len(NumEDATabsCurrent)) {
            ...
            if(data.table::is.data.table(dd)) {
              ColTypes <- unique(DataMuse:::ColTypes(dd))
              for(j in seq_along(ColTypes)) ChoiceList[[ColTypes[j]]] <- DataMuse:::ColNameFilter(dd, Types = ColTypes[j])
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('EDAData',i), Label = 'Data Selection', Choices = tryCatch(names(DataList), error = function(x) NULL), SelectedDefault = EDATabList[[paste0("EDAData",i)]], Multiple = TRUE, MaxVars = 1L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('EDAUnivariateVars',i), Label = 'Univariate Vars', Choices = ChoiceList, SelectedDefault = EDATabList[[paste0("EDAUnivariateVars",i)]], Multiple = TRUE, MaxVars = 100L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0("EDACorrVars", i), Label = 'Corr Vars', Choices = ChoiceList, SelectedDefault = EDATabList[[paste0("EDACorrVars",i)]], Multiple = TRUE, MaxVars = 100L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('EDATrendVars',i), Label = 'Trend Vars', Choices = ChoiceList, SelectedDefault = EDATabList[[paste0("EDATrendVars",i)]], Multiple = TRUE, MaxVars = 100L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0("EDADateVar", i), Label = 'Trend Date Var', Choices = ChoiceList, SelectedDefault = EDATabList[[paste0("EDADateVar",i)]], Multiple = TRUE, MaxVars = 1L)
              DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0('EDAGroupVar',i), Label = 'Trend By-Variable', Choices = ChoiceList, SelectedDefault = EDATabList[[paste0("EDAGroupVar",i)]], Multiple = TRUE, MaxVars = 1L)
            }
          ...
          if(length(PlotDropDown) > 0L) {
            for(i in seq_along(PlotDropDown)[-1L]) { # Debug is first element; i = 2
              ...
              if(length(selected_data) > 1L && data.table::is.data.table(selected_data)) {
                ...
                ChoiceList <- list();ChoiceListSelectize <- list()
                ColTypes <- unique(DataMuse:::ColTypes(selected_data))
                for(j in seq_along(ColTypes)) {
                  if(length(ColTypes) >= j) {
                    ChoiceList[[ColTypes[j]]] <- DataMuse:::ColNameFilter(selected_data, Types = ColTypes[j])
                    ChoiceListSelectize[[ColTypes[j]]] <- as.list(DataMuse:::ColNameFilter(selected_data, Types = ColTypes[j]))
                  }
                }
              ...
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("Plot",i-1L,"_SelectData"), Label = "Select Data", Choices = names(DataList), Multiple = TRUE, MaxVars = 1, SelectedDefault = PlotDropDown[[i]][[paste0("Plot",key,"_SelectData")]], Debug = DebugSessionLoad, CloseAfterSelect = TRUE)
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('YVar',i-1), Label = 'Y-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('YVar',i-1)]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('DualYVar',i-1), Label = 'Dual Axis Y-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('DualYVar',i-1)]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('XVar',i-1), Label = 'X-Variable', Choices = ChoiceListSelectize, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('XVar',i-1)]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('ZVar',i-1), Label = 'Z-Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('ZVar',i-1)]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('PlottingDateVar',i-1), Label = 'Date Variable', Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('PlottingDateVar',i-1)]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('GroupVars',i-1), Label = 'Grouping Variables', Choices = Choices, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('GroupVars',i-1)]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("YVarTrans",i-1), Label = "Y-Variable Transformation", Choices = NULL, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('YVarTrans',i-1)]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("DualYVarTrans",i-1), Label = "Dual Axis Y-Variable Transformation", Choices = NULL, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('DualYVarTrans',i-1)]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("XVarTrans",i-1), Label = "X-Variable Transformation", Choices = NULL, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('XVarTrans',i-1)]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("ZVarTrans",i-1), Label = "Z-Variable Transformation", Choices = NULL, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('ZVarTrans',i-1)]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("AggMethod",i-1), Label = "Aggregation Method", Choices = c('count','proportion','mean','meanabs','median','medianabs','sum','sumabs','sd','sdabs', 'skewness','skewnessabs', 'kurtosis','kurtosisabs','CoeffVar','CoeffVarabs'), SelectedDefault = PlotDropDown[[i]][[paste0('AggMethod',i-1)]])
              DataMuse:::SliderInput(session = session, Update = TRUE, input=input, InputID = paste0("FacetRows",i-1), Step = 1, Value = PlotDropDown[[i]][[paste0('FacetRows',i-1)]], Min = 1, Max = 12)
              DataMuse:::SliderInput(session = session, Update = TRUE, input=input, InputID = paste0("FacetCols",i-1), Step = 1, Value = PlotDropDown[[i]][[paste0('FacetCols',i-1)]], Min = 1, Max = 12)
              DataMuse:::NumericInput(session = session, Update = TRUE, input=input, InputID = paste0("PlottingMaxLags", i-1), Label = 'Max Lags', Value = PlotDropDown[[i]][[paste0('PlottingMaxLags',i-1)]], Min = 5, Max = 200, Step = 1)
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0("PlottingTimeUnit", i-1), Label = "Time Agg Level", Choices = c("hour", "day", "week", "month", "quarter", "year"), SelectedDefault = PlotDropDown[[i]][[paste0('PlottingTimeUnit',i-1)]], Multiple = TRUE, MaxVars = 1)
              sgs <- PlotDropDown[[paste0("GroupVars",i-1)]]
              if(length(sgs) > 0L) {
                DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = sgs[1L], InputID=paste0('Levels_',i-1,'_1'), Choices=Choices, SelectedDefault = PlotDropDown[[i]][[paste0('Levels_',i-1,'_1')]])
                if(length(sgs) > 1L) {
                  DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = sgs[2L], InputID=paste0('Levels_',i-1,'_2'), Choices=Choices, SelectedDefault = PlotDropDown[[i]][[paste0('Levels_',i-1,'_2')]])
                  if(length(sgs) > 2L) {
                    DataMuse:::PickerInput_GetLevels2(Update = TRUE, session = session, input=input, GroupVariable = sgs[3L], InputID=paste0('Levels_',i-1,'_3'), Choices=Choices, SelectedDefault = PlotDropDown[[i]][[paste0('Levels_',i-1,'_3')]])
                  }
                }
              }


              DataMuse:::TextInput(session = session, Update = TRUE, input=input, InputID = paste0('Title', i-1), Label='Rename Title', Value = PlotDropDown[[i]][[paste0('Title',i-1)]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('ShowLabels', i-1), Label='Show Labels', Choices = c(TRUE,FALSE), Multiple=TRUE, MaxVars = 1, SelectedDefault = PlotDropDown[[i]][[paste0('ShowLabels',i-1)]], CloseAfterSelect = TRUE)
              DataMuse:::TextInput(session = session, Update = TRUE, input=input, InputID = paste0("YAxisTitle", i-1), Label = "Y-Axis Title ('None' for blank)", Value = PlotDropDown[[i]][[paste0('YAxisTitle',i-1)]])
              DataMuse:::TextInput(session = session, Update = TRUE, input=input, InputID = paste0("XAxisTitle", i-1), Label = "X-Axis Title ('None' for blank)", Value = PlotDropDown[[i]][[paste0('XAxisTitle',i-1)]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',i-1,'_1'), Label = "Filter Variable", Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('FilterVariable_',i-1,'_1')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',i-1,'_2'), Label = "Filter Variable", Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('FilterVariable_',i-1,'_2')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',i-1,'_3'), Label = "Filter Variable", Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('FilterVariable_',i-1,'_3')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterVariable_',i-1,'_4'), Label = "Filter Variable", Choices = ChoiceList, Multiple = TRUE, SelectedDefault = PlotDropDown[[i]][[paste0('FilterVariable_',i-1,'_4')]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',i-1,'_1'), Label = "Logical Operation", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterLogic_',i-1,'_1')]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',i-1,'_2'), Label = "Logical Operation", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterLogic_',i-1,'_2')]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',i-1,'_3'), Label = "Logical Operation", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterLogic_',i-1,'_3')]])
              DataMuse:::SelectizeInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterLogic_',i-1,'_4'), Label = "Logical Operation", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterLogic_',i-1,'_4')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_1_1'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_1_1')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_1_2'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_1_2')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_2_1'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_2_1')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_2_2'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_2_2')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_3_1'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_3_1')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_3_2'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_3_2')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_4_1'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_4_1')]])
              DataMuse:::PickerInput(session = session, Update = TRUE, input=input, InputID = paste0('FilterValue_',i-1,'_4_2'), Label = "Value 1", Choices = Choices, Multiple = TRUE, MaxVars = 30, SelectedDefault = PlotDropDown[[i]][[paste0('FilterValue_',i-1,'_4_2')]])
            }
          }
          ...
          if(PlotMap[,.N] > 0L) {
            ...
            for(OutputTabNumber in seq_len(NumTabs)) {
              if(OutputTabNumber == 1L) {
                ...
                for(i in 1L:12L) {
                  if(PlotMap[i][["PlotName"]] != "") {
                    DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0("PlotID_", i), Choices = PlotList, SelectedDefault = PlotMap[i][["PlotType"]])
                  } else {
                    DataMuse::PickerInput(session = session, input = input, Update = TRUE, InputID = paste0("PlotID_", i), Choices = PlotList, SelectedDefault = NULL, PlotSelection = paste0("Plot",i))
                  }
                }
              ...
              sel <<- sel
              DataMuse:::SliderInput(session = session, Update = TRUE, server = server, input = input, InputID = paste0("PlotWidth",OutputTabNumber), Value = tryCatch({loadPlotPanelInputList[[paste0("PlotWidth",OutputTabNumber)]]}, error = function(x) 1450), PlotDropDown = loadPlotPanelInputList, Key = paste0("PlotWidth",OutputTabNumber), MainPanel = "bla")
              DataMuse:::SliderInput(session = session, Update = TRUE, server = server, input = input, InputID = paste0("PlotHeight",OutputTabNumber), Value = tryCatch({loadPlotPanelInputList[[paste0("PlotHeight",OutputTabNumber)]]}, error = function(x) 950), PlotDropDown = loadPlotPanelInputList, Key = paste0("PlotHeight",OutputTabNumber), MainPanel = "bla")
              DataMuse:::SliderInput(session = session, Update = TRUE, server = server, input = input, InputID = paste0("GridCols",OutputTabNumber), Value = tryCatch({loadPlotPanelInputList[[paste0("GridCols",OutputTabNumber)]]}, error = function(x) 1), PlotDropDown = loadPlotPanelInputList, Key = paste0("GridCols",OutputTabNumber), MainPanel = "bla")
              DataMuse:::SliderInput(session = session, Update = TRUE, server = server, input = input, InputID = paste0("Number_of_Bins",OutputTabNumber), Value = tryCatch({loadPlotPanelInputList[[paste0("Number_of_Bins",OutputTabNumber)]]}, error = function(x) 20), PlotDropDown = loadPlotPanelInputList, Key = paste0("Number_of_Bins",OutputTabNumber), MainPanel = "bla")
              DataMuse:::SliderInput(session = session, Update = TRUE, server = server, input = input, InputID = paste0("Number_of_Levels",OutputTabNumber), Value = tryCatch({loadPlotPanelInputList[[paste0("Number_of_Levels",OutputTabNumber)]]}, error = function(x) 75), PlotDropDown = loadPlotPanelInputList, Key = paste0("Number_of_Levels",OutputTabNumber), MainPanel = "bla")
              DataMuse:::SliderInput(session = session, Update = TRUE, server = server, input = input, InputID = paste0("FontSize",OutputTabNumber), Value = tryCatch({loadPlotPanelInputList[[paste0("FontSize",OutputTabNumber)]]}, error = function(x) 14), PlotDropDown = loadPlotPanelInputList, Key = paste0("FontSize",OutputTabNumber), MainPanel = "bla")
            }
          }
  ...
  shiny::observeEvent(input$ImportLocalCSV,    {
    ...
    if(length(Filepath) > 0L) {
      DataList[[basename(FilepathName)]][['data']] <- tryCatch({DataMuse:::LoadCSV(Filepath)}, error = function(x) NULL)
      if(length(DataList[[basename(FilepathName)]][['data']]) > 0L) {
        ...
        DataList <<- DataList
        DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
          "\n",
          "# Load CSV\n",
          "library(data.table)\n",
          "if(!exists('DataList')) DataList <- list()\n",
          "filename <- basename(", DataMuse:::CEP(FilepathName), ")\n",
    ...
    if(length(Filepath) > 0L) {
      ...
      if(!grepl(pattern = ".csv", x = filename)) {
        ...
        if(import_check) {
          ...
          # Code Collect
          DataMgtCode <- tryCatch({DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
            "\n",
            "# Load Model\n",
            "if(!exists(ModelOutputList)) ModelOutputList <- list()\n",
            "ModelOutputList[[", DataMuse:::CEP(filename), "]] <- readRDS(file.choose())\n"))}, error = function(x) DataMgtCode)

          # Add data to DataOutputSelection Page
          if(Debug) print("ML Reporting Tab Update")
          for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("MLReportsModelSelection", i), Label = 'Model Selection', Choices = names(ModelOutputList), Multiple = TRUE, MaxVars = 100L)
          for(i in seq_len(NumTabs)) DataMuse::PickerInput(session, input, Update = TRUE, InputID = paste0("DataOutputSelection", i), Label = 'Display Data', Choices = names(DataList), Multiple = TRUE, MaxVars = 100L)
          DataMuse::SelectizeInput(session, input, Update = TRUE, InputID = 'ScoreML_Model', Label = 'Select Model', Choices = names(ModelOutputList), SelectedDefault = NULL, Multiple = FALSE)
          DataMuse::SelectizeInput(session, input, Update = TRUE, InputID = 'ScoreML_Data', Label = 'Select Data', Choices = names(DataList), SelectedDefault = NULL, Multiple = FALSE)
  ...
  shiny::observeEvent(input$ImportLightGBMModel,  {
    ...
    if(length(Filepath) > 0L &&
      ...
      DataMgtCode <- tryCatch({DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        "\n",
        "# Load Model\n",
        "ModelOutputList[[", DataMuse:::CEP(LightGBM_ModelID), "]]$Model <- lightgbm::readRDS.lgb.Booster(file.choose())\n"))}, error = function(x) DataMgtCode)
  ...
  shiny::observeEvent(input$PostGRE_PullTable, {
    ...
    sql <- DataMuse:::ReturnParam(xx = tryCatch({input[['sql_code']]}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    PostGRE_PullTable_DB <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_PullTable_DB']]}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    PostGRE_PullTable_Table <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_PullTable_Table']]}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    PostGRE_SamplePercent <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_SamplePercent']]}, error = function(x) NULL), Type = 'numeric', Default = 1.0, Debug = Debug)
    if(length(sql) > 0L && nchar(sql) < 5) sql <- NULL

    # Query Builder
    if(Debug) print("PostGRE Pull Table 2")
    PostGRE_PullTable_Columns <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_PullTable_Columns']]}, error = function(x) NULL), Type = 'character', Default = '*', Debug = Debug)
    PostGRE_PullTable_GroupByColumns <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_PullTable_GroupByColumns']]}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    PostGRE_PullTable_AggStat <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_PullTable_AggStat']]}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    ...
    if(length(PostGRE_PullTable_DB) > 0L && length(PostGRE_PullTable_Table) > 0L) {
      ...
      # Pull Data
      if(Debug) print(sql)
      DataList[[PostGRE_PullTable_Table]][['data']] <- DataMuse:::DM.pgQuery(
        Query = sql,
      ...
      # Update DataList
      DataList <- DataMuse:::DM.DataListUpdate(DataList, PostGRE_PullTable_Table)
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        ...
        "# Query postgres\n",
        "library(data.table)\n",
        "DataList[[", DataMuse:::CEP(PostGRE_PullTable_Table), "]][['data']] <- DataMuse::DM.pgQuery(\n  ",
        "Query = ", sql, ",\n  ",
        "Host = ", DataMuse:::CEP(PostGRE_Host), ",\n  ",
        "DataBase = ", DataMuse:::CEP(PostGRE_PullTable_DB), ",\n  ",
        "SELECT = ", DataMuse:::CEP(PostGRE_PullTable_Columns), ",\n  ",
        "AggStat = ", DataMuse:::CEP(PostGRE_PullTable_AggStat), ",\n  ",
        "FROM = ", DataMuse:::CEP(PostGRE_PullTable_Table), ",\n  ",
        "GroupBy = ", DataMuse:::CEP(PostGRE_PullTable_GroupByColumns), ",\n  ",
        "SamplePercent = ", DataMuse:::CEPP(PostGRE_SamplePercent), ",\n  ",
        "User = ", DataMuse:::CEP(PostGRE_User), ",\n  ",
        "Port = ", DataMuse:::CEPP(PostGRE_Port), ",\n  ",
        "Password = ", DataMuse:::CEP(PostGRE_Password), ")\n"))
  ...
  shiny::observeEvent(input$PostGRE_CreateDB,  {
    ...
    if(length(PostGRE_NewDB_DB) > 0L && PostGRE_NewDB_DB != "") {
      tryCatch({DataMuse::DM.pgCreateDB(
        Host = PostGRE_Host,
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        ...
        "DataMuse::DM.pgCreateDB(\n  ",
        "Host = ", DataMuse:::CEP(PostGRE_Host), ",\n  ",
        "DataBase = ", DataMuse:::CEP(PostGRE_NewDB_DB), ",\n  ",
        "User = ", DataMuse:::CEP(PostGRE_User), ",\n  ",
        "Port = ", DataMuse:::CEP(PostGRE_Port), ",\n  ",
        "Password = ", DataMuse:::CEP(PostGRE_Password), ")\n"))
      ...
      output$PostGRE_PullTable_DB <- shiny::renderUI({
        DataMuse:::SelectizeInput(InputID = 'PostGRE_PullTable_DB', Label = NULL, Choices = PostGRE_DBNames, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1L, CloseAfterSelect = FALSE, Debug = Debug)
      })
      output$PostGRE_NewDB_DB <- shiny::renderUI({
        DataMuse:::SelectizeInput(InputID = 'PostGRE_NewDB_DB', Label = NULL, Choices = PostGRE_DBNames, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1L, CloseAfterSelect = FALSE, Debug = Debug)
      })
      output$PostGRE_DropTable_DB <- shiny::renderUI({
        DataMuse:::SelectizeInput(InputID = 'PostGRE_DropTable_DB', Label = NULL, Choices = PostGRE_DBNames, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1L, CloseAfterSelect = FALSE, Debug = Debug)
      })
      output$PostGRE_DropDB_DB <- shiny::renderUI({
        DataMuse:::SelectizeInput(InputID = 'PostGRE_DropDB_DB', Label = NULL, Choices = PostGRE_DBNames, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1L, CloseAfterSelect = FALSE, Debug = Debug)
      })
  ...
  shiny::observeEvent(input$PostGRE_DropTable, {
    ...
    # Local PostGRE Data
    PostGRE_DropTable_DB <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_DropTable_DB']]}, error = function(x) NULL), Type = 'character', Default = NULL)
    PostGRE_DropTable_Table <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_DropTable_Table']]}, error = function(x) NULL), Type = 'character', Default = NULL)

    # Drop Table
    if(length(PostGRE_DropTable_Table) > 0L && length(PostGRE_DropTable_DB) > 0L) {
      for(i in PostGRE_DropTable_Table) {
        x <- tryCatch({DataMuse::DM.pgRemovePlotTable(
          Table = i,
          Host = PostGRE_Host,
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        "\n",
        "# Remove Table\n",
        "for(i in c(", DataMuse:::ExpandText(PostGRE_DropTable_Table), ")) {\n  ",
        "DataMuse::DM.pgRemovePlotTable(\n    ",
        "Table = i,\n    ",
        "Host = ", DataMuse:::CEP(PostGRE_Host), ",\n    ",
        "DataBase = i,\n    ",
        "User = ", DataMuse:::CEP(PostGRE_User), ",\n    ",
  ...
  shiny::observeEvent(input$PostGRE_DropDB,    {
    ...
    # Local PostGRE Data
    DropDatabaseName <- DataMuse:::ReturnParam(xx = tryCatch({input[['PostGRE_DropTable_DB']]}, error = function(x) NULL), Type = 'character', Default = NULL)
    if(length(DropDatabaseName) > 0L) {
      for(i in DropDatabaseName) {
        tryCatch({
          DataMuse::DM.pgDropDB(
            Host = PostGRE_Host,
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        "\n",
        "# Create new PostGRE Database\n",
        "for(i in c(", DataMuse:::ExpandText(DropDatabaseName), ")) {\n  ",
        "DataMuse::DM.pgDropDB((\n    ",
        "Host = ", DataMuse:::CEP(PostGRE_Host), ",\n    ",
        "DataBase = i,\n    ",
        "User = ", DataMuse:::CEP(PostGRE_User), ",\n    ",
      ...
      # Update available databases
      if(length(PostGRE_DBNames) == 0L) {
        PostGRE_DBNames <- tryCatch({DataMuse::DM.pgListDatabases(Host = PostGRE_Host, Port = PostGRE_Port, User = PostGRE_User, Password = PostGRE_Password)}, error = function(x) NULL)
      }
      DataMuse:::SelectizeInput(session=session, Update = TRUE, input = input, InputID = 'PostGRE_PullTable_DB', Label = NULL, Choices = PostGRE_DBNames, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1L, CloseAfterSelect = FALSE, Debug = Debug)
      DataMuse:::TextInput(session=session, Update = TRUE, input = input, InputID = 'PostGRE_NewDB_DB', Label = NULL, Value = NULL)
      DataMuse:::SelectizeInput(session=session, Update = TRUE, input = input, InputID = 'PostGRE_DropTable_DB', Label = NULL, Choices = input$LocalPostGRE_Database, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1, CloseAfterSelect = FALSE, Debug = Debug)
      DataMuse:::SelectizeInput(session=session, Update = TRUE, input = input, InputID = 'PostGRE_DropDB_DB', Label = NULL, Choices = input$LocalPostGRE_Database, SelectedDefault = NULL, Multiple = TRUE, MaxVars = 1, CloseAfterSelect = FALSE, Debug = Debug)
    ...
    if(length(FileName) != 0 && FileName != "Load" && FileName != "") {
      ...
      if(length(x) > 0L) {
        ...
        for(i in seq_len(24L)) {
          DataList[[filename]] <<- tryCatch({DataMuse:::LoadCSV(Infile = file.path('/inputdata', input[['AzureBlobStorageTabular']]), Debug = Debug)}, error = function(x) NULL)
          if(length(DataList[[filename]]) > 0L) {
            xx <- tryCatch({DataList[[PostGRE_PullTable_Table]][['data']][,.N]}, error = function(x) NULL)
            if(length(xx) > 0L) {
              DataList <- tryCatch({DataMuse:::DM.DataListUpdate(DataList, PostGRE_PullTable_Table)}, error = function(x) DataList)
            }
            break
          } else {
            Sys.sleep(5)
        ...
        DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
          ...
          "x <- ", DataMuse:::CEP(x), "\n",
          "filename <<- basename(", DataMuse:::CEP(filename), "\n",
          "ModelOutputList <- readRDS(file.path('/inputdata', x)),\n",
          "DataList[[filename]] <- DataMuse:::LoadCSV(Infile = file.path('/inputdata', input[['AzureBlobStorageTabular']]))\n"))
        ...
        for(i in seq_len(24L)) {
          ...
          if(length(ModelOutputList) > 0L) {
            if(!is.null(ModelOutputList$TrainData) && !is.null(ModelOutputList$TestData)) {
              ...
              if(length(xx) > 0L) {
                DataList <- tryCatch({DataMuse:::DM.DataListUpdate(DataList, filename)}, error = function(x) DataList)
              }
              DataList <<- DataList
              ...
              DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
                "\n",
                "# Load ModelOutputList\n",
                "AzureStor::download_blob(container = ", DataMuse:::CEP(cont),", src = ", DataMuse:::CEP(input[['AzureBlobStorageRdata']]), ", dest = ", DataMuse:::CEP(file.path('/inputdata', input[['AzureBlobStorageRdata']])), ", overwrite=TRUE)\n",
                "x <- ", DataMuse:::CEP(x), "\n",
                "filename <- basename(x)\n",
                "ModelOutputList <- readRDS(file.path('/inputdata', x)),\n",
  ...
  shiny::observeEvent(input$SaveLocalData,     {
    SaveData <- DataMuse:::ReturnParam(xx = tryCatch({shiny::req(input$SaveData_SelectData)}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    SaveFileName <- DataMuse:::ReturnParam(xx = tryCatch({shiny::req(input$SaveFileName)}, error = function(x) NULL), Type = 'character', Default = NULL, Debug = Debug)
    ...
    if(length(SaveFileName) > 0L && length(SaveData) > 0L) {
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        ...
        "data.table::fwrite(DataList[[", DataMuse:::CEP(SaveData), "]], file = file.path(", DataMuse:::CEP(WorkingDirectory), ", paste0(", DataMuse:::CEP(SaveFileName), ", '.csv')))\n"))
  ...
  shiny::observeEvent(input$SaveLocalDataRdata,{
    ...
    if(length(SaveFileName) > 0L && length(SaveData) > 0L) {
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        ...
        "save(ModelOutputList[[", DataMuse:::CEP(SaveData), "]], file = file.path(", DataMuse:::CEP(LocalExportPath), ", paste0(", DataMuse:::CEP(SaveData), ", '.rds')))\n"
      ))
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = 'Success', btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "20%")
    } else {
      shinyWidgets::sendSweetAlert(session, title = NULL, text = NULL, type = NULL, btn_labels = 'Missing File Name', btn_colors = NULL, html = FALSE, closeOnClickOutside = TRUE, showCloseButton = TRUE, width = "20%")
  ...
  shiny::observeEvent(input$PostGRE_Push,      {
    ...
    # Data Name
    TableName <- DataMuse:::ReturnParam(xx=tryCatch({input[['SaveData_TableName']]}, error=function(x) NULL), Type='character', Default=NULL)
    DataBaseName <- DataMuse:::ReturnParam(xx=tryCatch({input[['SaveData_DataBaseName']]}, error=function(x) NULL), Type='character', Default=NULL)
    ...
    if(length(TableName) > 0L && length(DataBaseName) > 0L && data.table::is.data.table(postgre_data)) {
      tryCatch({DataMuse::DM.pgRemoveCreateAppend(
        data = postgre_data,
      ...
      DataMgtCode <- DataMuse::Shiny.CodePrint.Collect(y = DataMgtCode, x = paste0(
        ...
        "\n",
        "DataMuse::DM.pgRemoveCreateAppend(\n  ",
        "data = ", DataMuse:::CEP(postgre_data), ",\n  ",
        "DataBase = ", DataMuse:::CEP(DataBaseName), ",\n  ",
        "Tabl = ", DataMuse:::CEP(TableName), ",\n  ",
        "Host = ", DataMuse:::CEP(PostGRE_Host), ",\n  ",
        "User = ", DataMuse:::CEP(PostGRE_User), ",\n  ",
        "Port = ", DataMuse:::CEP(PostGRE_Port), ",\n  ",
        "Password = ", DataMuse:::CEP(PostGRE_Password), ")\n"))
  ...
  shiny::observeEvent(input$PrintCodeButton, {
    ...
    CodePrintFontSize <- DataMuse::ReturnParam(xx = input$CodePrintFontSize, Type = "numeric", Default = 16)
  ...
  shiny::observeEvent({eval(EDAOutputExpression)}, {
    ...
    if(length(DataList) > 0L) {
      shiny::withProgress(message = 'EDA Reporting Has Begun..', value = 0, {
        ...
        if(Debug) {
          Output <- DataMuse:::Shiny.EDA.ReportOutput(
          ...
          Output <- tryCatch({
            DataMuse:::Shiny.EDA.ReportOutput(
              input, output, DataList, MachineLearningCode, Page,
              Debug = Debug,
              Theme = EchartsTheme,
              FontColor = FontColorData)
  ...
  shiny::observeEvent({eval(EDAReportOutputExpression)}, {
    ...
    if(length(DataList) > 0L) {
      shiny::withProgress(message = 'EDA Reporting Has Begun..', value = 0, {
        ...
        # Inputs
        EchartsTheme <- DataMuse:::ReturnParam(xx = input[["EchartsTheme"]], Type = "character", Default = "dark")
        EDADataHTML <- DataList[[DataMuse:::ReturnParam(xx = tryCatch({input[[paste0("EDAData", Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)]][['data']]
        ...
        MachineLearningCode <- DataMuse:::Shiny.CodePrint.Collect(y = MachineLearningCode, x = paste0(
          ...
          "DataMuse:::EDAReport(\n\n  ",
          "DataName = ", DataMuse:::CEP(input[[paste0("EDAData", Page)]]), ",\n  ",
          "UnivariateVars = ", DataMuse:::ExpandText(UnivariateVars), ",\n  ",
          "CorrVars = ", DataMuse:::ExpandText(CorrVars), ",\n  ",
          "TrendVars = ", DataMuse:::ExpandText(TrendVars), ",\n  ",
          "TrendDateVar = ", DataMuse:::CEP(TrendDateVar), ",\n  ",
          "TrendGroupVar = ", DataMuse:::CEP(TrendGroupVar), ",\n  ",
  ...
  shiny::observeEvent({eval(MLOutputExpression)}, {
    ...
    if(length(ModelOutputList) > 0L) {
      shiny::withProgress(message = 'ML Reporting Has Begun..', value = 0, {
        ...
        # Run DataMuse:::Shiny.ML.ReportOutput
        if(Debug) {
          MLOut <- DataMuse:::Shiny.ML.ReportOutput(
            input, output, DataList, MachineLearningCode, Page,
            Debug = Debug,
            MOL = ModelOutputList,
          ...
          MLOut <- tryCatch({
            DataMuse:::Shiny.ML.ReportOutput(
              input, output, DataList, MachineLearningCode, Page,
              Debug = Debug,
              MOL = ModelOutputList,
              ModelID = MLModelSelection,
  ...
  shiny::observeEvent({eval(MLReportOutputExpression)}, {
    ...
    if(length(ModelOutputList) > 0L) {
      shiny::withProgress(message = 'ML Report Exporting Has Begun..', value = 0, {
        ...
        # Inputs
        ModelID <- DataMuse:::ReturnParam(xx = tryCatch({input[[paste0('MLReportsModelSelection',Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)
        FeatureColNames <- DataMuse:::ReturnParam(xx = tryCatch({input[[paste0('PDPVariables',Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)
        SampleSize <- DataMuse:::ReturnParam(xx = tryCatch({input[[paste0('SampleSize',Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)
        TrainDataInclude <- DataMuse:::ReturnParam(xx = tryCatch({input[[paste0('TrainDataInclude',Page)]]}, error = function(x) NULL), Type = "character", Default = NULL)
        ...
        MachineLearningCode <- DataMuse:::Shiny.CodePrint.Collect(y = MachineLearningCode, x = paste0(
          ...
          "# ML Build Model\n",
          "DataMuse::ModelInsightsReport(\n\n  ",
          "FeatureColumnNames = ", DataMuse:::ExpandText(FeatureColNames), ",\n  ",
  ...
  shiny::observeEvent({eval(DataOutputExpression)}, {
    ...
    if(length(DataList) > 0L) {
      ...
      if(length(DataOuts) > 0L && any(DataOuts != "No Data Loaded")) {
        ...
        GridColsData <- DataMuse::ReturnParam(xx = input[[paste0("GridColsData", Page)]], Type = "numeric", Default = 1)
        MinRows <- DataMuse::ReturnParam(xx = input[[paste0("MinRows", Page)]], Type = "numeric", Default = 10)
        Number_of_Records <- DataMuse::ReturnParam(xx = input[[paste0("Number_of_Records", Page)]], Type = "numeric", Default = 1000)
        Shuffle <- DataMuse::ReturnParam(xx = input[[paste0("Shuffle", Page)]], Type = "logical", Default = FALSE)
        WrapText <- DataMuse:::ReturnParam(xx = input[[paste0("WrapText", Page)]], Type = "logical", Default = FALSE)
        Compact <- DataMuse:::ReturnParam(xx = input[[paste0("Compact", Page)]], Type = "logical", Default = FALSE)
  ...
  shiny::observeEvent({eval(PlotExecuteExpression)}, {
    ...
    if(length(DisplayRequests) > 0L) {
      ...
      out <- DataMuse:::PlotSize_(input, TabNum, nc, plotnumbers_temp)
      ...
      gtl[["PE"]] <- DataMuse:::ReturnParam(xx = tryCatch({input[["PlottingFramework"]]}, error = function(x) "Echarts"), Type = "character", Default = "Echarts")
      cf3 <- DataMuse:::rgba2hex(DataMuse:::ReturnParam(xx = input[["ColorFont"]], Type = "character", Default = "#e2e2e2"))
      gtl[["ColorFont"]] <- cf3$flv
      gtl[["TL"]] <- DataMuse:::ReturnParam(xx = input[["EchartsTimeLine"]],Type = "logical",Default = FALSE, Debug = FALSE)
      gtl[["EchartsTheme"]] <- DataMuse:::ReturnParam(xx = input[["EchartsTheme"]], Type = "character", Default = "macarons")
      gtl[["NumLevelsDisplay"]] <- DataMuse:::ReturnParam(xx = input[[paste0("Number_of_Levels", TabNum)]], Type = "numeric", Default = 20L, Debug = DebugPlottingOE)
      gtl[["FontSize"]] <- DataMuse:::ReturnParam(xx = input[[paste0("FontSize", TabNum)]], Type = "numeric", Default = 14L, Debug = DebugPlottingOE)
      gtl[["Number_of_Bins"]] <- DataMuse:::ReturnParam(xx = input[[paste0("Number_of_Bins", TabNum)]], Type = "numeric", Default = 21L, Debug = DebugPlottingOE)
      ...
      if(length(PlotNumbers) > 0L) {
        ...
        # Build plots
        if(DebugPlottingOE) {print("DataMuse:::Shiny.Plot.Build :::::::::::::::::::::::::::::::::");print(PlotNumbers);print(TabNum);print(NumPlotsAvailable); print(length(PlotDropDown))}
        PlotOutput <- tryCatch({DataMuse:::Shiny.Plot.Build(
          input,output,session,
      ...
      if(length(PlotOutput) > 0L) {
        ...
        out <- DataMuse:::PlotSize_(input, 1, 1, 1)
        if(length(out) == 0L) {
