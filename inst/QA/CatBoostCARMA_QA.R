...
for(run in seq_len(QA_Results[,.N])) {
  ...
  if(length(TestModel) > 0L) {
    if(length(TestModel$Forecast) > 0) {
      QA_Results <- QA_Results[eval(run), MeanPred := TestModel$Forecast[is.na(`Daily Margin`), round(mean(Predictions))]]
      QA_Results <- QA_Results[eval(run), SDPred := TestModel$Forecast[is.na(`Daily Margin`), round(sd(Predictions))]]
    }
