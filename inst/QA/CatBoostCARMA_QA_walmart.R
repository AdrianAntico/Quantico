...
for(run in seq_len(QA_Results[,.N])) {
  ...
  data <- data.table::fread("C:/Users/Bizon/Documents/GitHub/rappwd/PanelFC_Data.csv")
  data <- data[Date < "2022-01-01"]

  if(QA_Results[run][["Group"]] == 0L) {
    GroupVars <- NULL
    data <- data[, list(Weekly_Sales = mean(Weekly_Sales)), by = "Date"]
  } else if(QA_Results[run][["Group"]] == 1L) {
    GroupVars <- "Store"
    data <- data[, list(Weekly_Sales = mean(Weekly_Sales)), by = c("Store", "Date")]
  } else if(QA_Results[run][["Group"]] == 2L) {
    GroupVars <- c("Store",'Dept')
  }

  ValidationData <- data.table::copy(data)
  data <- data[Date < "2012-08-10"]
  ...
  # Build forecast ----
  TestModel <- tryCatch({AutoQuant::AutoCatBoostCARMA(
    ...
    Lags = c(1,7), #1, # list("weeks" = c(1:5), "months" = c(1:3)),
    MA_Periods = 7, #7,  #list("weeks" = c(2:5), "months" = c(2,3)),
    SD_Periods = NULL,
    Skew_Periods = NULL,
    Kurt_Periods = NULL,
  ...
  if(length(TestModel) > 0L) {
    if(length(TestModel$Forecast) > 0) {
      if(length(GroupVars) == 0) {
        ValidationData <- ValidationData[, list(Weekly_Sales = mean(Weekly_Sales)), by = "Date"]
        ValidationData <- merge(ValidationData, TestModel$Forecast[, .SD, .SDcols = c("Date", "Predictions")], by = c("Date"), all = FALSE)
      } else if(length(GroupVars) == 1) {
        ValidationData <- ValidationData[, list(Weekly_Sales = mean(Weekly_Sales)), by = c("Store", "Date")]
        ValidationData <- merge(ValidationData, TestModel$Forecast[, .SD, .SDcols = c("Store","Date", "Predictions")], by = c("Store","Date"), all = FALSE)
      } else {
        ValidationData <- merge(ValidationData, TestModel$Forecast[, .SD, .SDcols = c("Store","Dept","Date", "Predictions")], by = c("Store","Dept","Date"), all = FALSE)
      }
      ValidationData <- ValidationData[Date >= "2012-08-10"]
      ValidationData[, MAE := abs(Weekly_Sales - Predictions)]
      MAEE <- ValidationData[, mean(MAE)]
      QA_Results <- QA_Results[eval(run), MeanPred := TestModel$Forecast[is.na(Weekly_Sales), round(mean(Predictions))]]
      QA_Results <- QA_Results[eval(run), SDPred := TestModel$Forecast[is.na(Weekly_Sales), round(sd(Predictions))]]
      QA_Results <- QA_Results[eval(run), MAE := eval(MAEE)]
    }
  ...
  # Outcome
  rm(TestModel)
  AutoQuant:::Post_Append_Helper(QA_Results,'AutoCatBoostCARMA_QA')
  Sys.sleep(5)
}
...
FourierTerms = 0
TimeTrendVariable = FALSE
ZeroPadSeries = 'dynamic:credibility'
DataTruncate = FALSE
GridTune = FALSE
PassInGrid = NULL
ModelCount = 5
